...
def listar_planos():
    ...
    try:
        ...
        print("="*100)
        print(f"{'ID':<5} {'EMPRESA':<30} {'NOME DO PLANO':<35} {'TIPO':<15} {'CRIADO EM'}")
        print("-"*100)
        ...
        for row in rows:
            ...
            company_name = (company_name[:27] + '...') if len(company_name) > 30 else company_name
            created_at = created_at[:19] if len(created_at) > 19 else created_at
            
            print(f"{plan_id:<5} {company_name:<30} {plan_name:<35} {plan_mode:<15} {created_at}")
...
def atualizar_plan_mode(plan_id: int, novo_tipo: str):
    ...
    try:
        ...
        # Atualizar
        if hasattr(db, '_db_type') and db._db_type == 'postgresql':
            cursor.execute("UPDATE plans SET plan_mode = %s WHERE id = %s", (novo_tipo, plan_id))
        else:
            cursor.execute("UPDATE plans SET plan_mode = ? WHERE id = ?", (novo_tipo, plan_id))
...
def menu_interativo():
    ...
    if opcao == "1":
        ...
        ids = input("\nDigite os IDs dos planos separados por vírgula (ex: 1,2,3): ").strip()
        ...
        tipo = input("\nDigite o tipo para TODOS (evolucao ou implantacao): ").strip().lower()
