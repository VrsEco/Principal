...
def purge_companies_except_versus():
    try:
        ...
        if is_production and not allow_danger:
            return jsonify({ 'success': False, 'message': 'Operação desabilitada em produção. Defina ALLOW_DANGEROUS_OPS=true para habilitar.' }), 403
        ...
        if phrase.strip().upper() != expected_phrase:
            return jsonify({ 'success': False, 'message': f'Digite a frase de confirmação: "{expected_phrase}"' }), 400
        ...
        if not versus_ids:
            return jsonify({ 'success': False, 'message': 'Empresa Versus não encontrada. Configure VERSUS_COMPANY_ID ou cadastre a empresa.' }), 400

        # Delete plans (cascade) and companies not in Versus set
        to_delete_companies = Company.query.filter(~Company.id.in_(list(versus_ids))).all()
        ...
        try:
            ...
            pass
        return jsonify({ 'success': False, 'message': f'Erro ao executar limpeza: {str(exc)}' }), 500
...
def _plan_for(plan_id):
    ...
    company = {
        'id': plan_data['company_id'],
        'name': plan_data['company_name']
    }
...
def _navigation(plan_id, active_section):
    ...
    return [
        {'id': 'dashboard', 'name': 'Dashboard', 'url': f'/plans/{plan_id}', 'active': active_section == 'dashboard'},
        {'id': 'company', 'name': 'Dados da Organização', 'url': f'/plans/{plan_id}/company', 'active': active_section == 'company'},
        {'id': 'participants', 'name': 'Participantes', 'url': f'/plans/{plan_id}/participants', 'active': active_section == 'participants'},
        {'id': 'drivers', 'name': 'Direcionadores', 'url': f'/plans/{plan_id}/drivers', 'active': active_section == 'drivers'},
        {'id': 'okr-global', 'name': 'OKRs Globais', 'url': f'/plans/{plan_id}/okr-global', 'active': active_section == 'okr-global'},
        {'id': 'okr-area', 'name': 'OKRs de Área', 'url': f'/plans/{plan_id}/okr-area', 'active': active_section == 'okr-area'},
        {'id': 'projects', 'name': 'Projetos', 'url': f'/plans/{plan_id}/projects', 'active': active_section == 'projects'},
        {'id': 'reports', 'name': 'Relatórios', 'url': f'/plans/{plan_id}/reports', 'active': active_section == 'reports'}
    ]
...
def _indicator_navigation(company_id: Optional[int]) -> List[Dict[str, Any]]:
    ...
    try:
        from modules.grv import grv_navigation  # Lazy import to avoid circular dependencies
...
def _build_participant_context(plan_id: int) -> Tuple[List[Dict[str, Any]], Dict[str, str]]:
...
def _normalize_area_okr_payload(records: List[Dict[str, Any]], participant_lookup: Dict[str, str], indicator_lookup: Dict[str, str]) -> List[Dict[str, Any]]:
        ...
        for kr in item.get('key_results', []):
            ...
            if indicator_id_val is not None and indicator_id_val != '':
                ...
                if normalized_kr.get('indicator_label'):
                    normalized_kr['indicator_display'] = normalized_kr['indicator_label']
...
def _load_directionals(plan_id: int):
    ...
    for idx, directional in enumerate(raw_directionals):
        ...
        description = directional.get('description') or directional.get('insight') or directional.get('notes') or ''
        if not title and not description:
            continue

        item = directional.copy()
        norm_id = item.get('id') or item.get('directional_id') or item.get('uuid') or title or f'directional-{plan_id}-{idx}'
        norm_id = str(norm_id)
    ...
    try:
        ...
        raw_directionals = directionals_data.get('directionals', []) if isinstance(directionals_data, dict) else []
        ...
        if isinstance(raw_directionals, list):
            ...
            for idx, directional in enumerate(raw_directionals):
                if not isinstance(directional, dict):
                    continue
                item = directional.copy()
                norm_id = item.get('id') or item.get('directional_id') or item.get('uuid') or item.get('title')
                if not norm_id:
        ...
        for model in raw_models:
            if model:  # Verifica se o modelo não é None
                ...
                try:
                    ...
                    margin_bottom = int(margin_bottom) if margin_bottom is not None else 15
                ...
                try:
                    header_height = int(header_height) if header_height is not None else 25
                    footer_height = int(footer_height) if footer_height is not None else 12
                except (ValueError, TypeError):
                    header_height = 25
                    footer_height = 12
...
def api_generate_report_from_template(template_id):
    ...
    try:
        ...
        result = generator.generate_report_from_template(template_id, data.get('data_context', {}))
...
def company_logos_manager(company_id: int):
    ...
    return render_template(
        "company_logos_manager.html",
        company=company_data,
        logo_configs=logo_configs
    )
...
def api_upload_company_logo(company_id: int):
    ...
    try:
        ...
        if not logo_type or logo_type not in ['square', 'vertical', 'horizontal', 'banner']:
...
def api_update_company_mvv(company_id: int):
    try:
        payload = request.get_json(silent=True) or {}
        ok = db.update_company_mvv(company_id, payload.get('mission',''), payload.get('vision',''), payload.get('values',''))
        if ok:
            return jsonify({'success': True})
        return jsonify({'success': False, 'error': 'update_failed'}), 400
    except Exception as _err:
...
def api_create_company():
    ...
    try:
        ...
        if raw_client_code is None:
            return jsonify({'success': False, 'error': 'Código do cliente é obrigatório'}), 400

        client_code = raw_client_code.strip().upper()
        if not re.fullmatch(r'[A-Z0-9]{1,3}', client_code):
            return jsonify({'success': False, 'error': 'Código do cliente deve ter de 1 a 3 caracteres (letras ou números)'}), 400
...
def api_create_plan():
    ...
    try:
        ...
        name = (payload.get('name') or '').strip()
        if not name:
            return jsonify({'success': False, 'error': 'Nome do planejamento é obrigatório'}), 400
        ...
        if not end_date_str:
            return jsonify({'success': False, 'error': 'Data de fim é obrigatória'}), 400
        ...
        if end_date < start_date:
            return jsonify({'success': False, 'error': 'Data de fim deve ser posterior à data de início'}), 400
...
def api_gerar_relatorio_projetos(company_id: int):
    ...
    try:
        ...
        return jsonify({
            ...
            'pdf_path': pdf_path,
            'engine': engine,
            'download_url': url_for('gerar_relatorio_projetos', company_id=company_id, _external=True)
        })
...
def api_company_employee(company_id: int, employee_id: int):
    ...
    if request.method == 'DELETE':
        try:
            ...
            if success:
                return jsonify({'success': True, 'message': 'Colaborador excluído com sucesso.'})
            else:
        ...
        for emp_row in cursor.fetchall():
            ...
            for routine_row in cursor.fetchall():
                ...
                if schedule_type == 'daily':
                    ...
                    try:
                        if schedule_value:
                            days_data = json.loads(schedule_value) if isinstance(schedule_value, str) else schedule_value
            ...
            utilization = round((total_weekly_hours / weekly_capacity * 100) if weekly_capacity > 0 else 0, 1)
...
@app.route('/api/companies/<int:company_id>/process-areas/<int:area_id>', methods=['PUT'])
...
@app.route('/api/companies/<int:company_id>/process-areas/<int:area_id>', methods=['DELETE'])
...
@app.route('/api/companies/<int:company_id>/macro-processes/<int:macro_id>', methods=['PUT'])
...
@app.route('/api/companies/<int:company_id>/macro-processes/<int:macro_id>', methods=['DELETE'])
...
@app.route('/api/companies/<int:company_id>/processes/<int:process_id>', methods=['PUT'])
...
@app.route('/api/companies/<int:company_id>/processes/<int:process_id>/stage', methods=['PATCH'])
...
@app.route('/api/companies/<int:company_id>/processes/<int:process_id>/notes', methods=['GET', 'PUT'])
...
def _allowed_flow_extension(filename: str) -> bool:
    return '.' in filename and filename.rsplit('.', 1)[1].lower() in PROCESS_FLOW_ALLOWED_EXTENSIONS
...
def _allowed_activity_image_extension(filename: str) -> bool:
    return '.' in filename and filename.rsplit('.', 1)[1].lower() in PROCESS_ACTIVITY_ALLOWED_EXTENSIONS
...
def _build_activity_code(process_code: Optional[str], process_id: int, suffix: str) -> str:
...
@app.route('/api/companies/<int:company_id>/processes/<int:process_id>/flow', methods=['POST', 'DELETE'])
...
def _serialize_activity_entry(entry: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        'image_url': url_for('serve_uploaded_file', filename=image_path) if image_path else None,
...
@app.route('/api/companies/<int:company_id>/processes/<int:process_id>/activities', methods=['GET', 'POST'])
def api_process_activities(company_id: int, process_id: int):
    ...
    new_id = db.create_process_activity(process_id, {
        'code_suffix': suffix,
        'name': name,
...
@app.route('/api/companies/<int:company_id>/processes/<int:process_id>/activities/<int:activity_id>', methods=['PUT', 'DELETE'])
def api_process_activity_detail(company_id: int, process_id: int, activity_id: int):
    ...
    if suffix_value is None:
        normalized_suffix = activity.get('code_suffix') or _extract_activity_suffix(activity.get('code'))
    else:
        ...
        for act in existing:
            ...
            if (_extract_activity_suffix(act.get('code')) == normalized_suffix):
                return jsonify({'success': False, 'error': 'duplicate_suffix'}), 400
    ok = db.update_process_activity(activity_id, {
        'name': name,
        'layout': layout if layout in ('single', 'dual') else 'single',
...
@app.route('/api/companies/<int:company_id>/processes/<int:process_id>/activities/<int:activity_id>/entries', methods=['POST'])
...
@app.route('/api/companies/<int:company_id>/processes/<int:process_id>/activities/<int:activity_id>/entries/<int:entry_id>', methods=['PUT', 'DELETE'])
def api_process_activity_entry_detail(company_id: int, process_id: int, activity_id: int, entry_id: int):
    ...
    text_content = (request.form.get('text_content') or entry.get('text_content') or '').strip()
    image_file = request.files.get('image')
    ...
    try:
        order_val = int(order_index) if order_index is not None else entry.get('order_index') or 0
    except ValueError:
        order_val = entry.get('order_index') or 0

    try:
        width_val = int(image_width) if image_width is not None else entry.get('image_width') or 280
        # Limita entre 180px e 600px
        width_val = max(180, min(600, width_val))
    except ValueError:
        width_val = entry.get('image_width') or 280

    ok = db.update_process_activity_entry(entry_id, {
        'text_content': text_content,
        'image_path': image_path,
...
@app.route('/api/companies/<int:company_id>/processes/<int:process_id>', methods=['DELETE'])
    ...
    try:
        ...
        try:
            ...
            if routine_row:
                ...
                if assigned_roles_json:
                    assigned_roles = json.loads(assigned_roles_json) if isinstance(assigned_roles_json, str) else assigned_roles_json
        ...
        cursor.execute(
            ...
            (
                ...
                json.dumps(assigned_collaborators), estimated_hours, trigger_type,
...
@app.route('/api/companies/<int:company_id>/processes/<int:process_id>/routine-collaborators', methods=['GET'])
...
@app.route('/api/companies/<int:company_id>/process-instances/<int:instance_id>', methods=['PATCH'])
...
def api_company_occurrences(company_id: int):
    ...
    if occ_type not in ['positive', 'negative']:
        return jsonify({'success': False, 'message': 'Tipo deve ser "positive" ou "negative".'}), 400
...
@app.route('/api/companies/<int:company_id>/occurrences/<int:occurrence_id>', methods=['PUT', 'DELETE'])
...
def api_create_routine_trigger(routine_id: int):
    ...
    try:
        ...
        if deadline_unit not in ['hours', 'days']:
            return jsonify({'success': False, 'error': 'Unidade de prazo inválida'}), 400
        
        trigger_id = db.create_routine_trigger(
            routine_id, trigger_type, trigger_value, 
            int(deadline_value), deadline_unit
        )
...
def api_update_routine_trigger(trigger_id: int):
    ...
    try:
        ...
        success = db.update_routine_trigger(
            trigger_id, trigger_type, trigger_value,
            int(deadline_value), deadline_unit
        )
        ...
        if success:
            ...
            return jsonify({'success': False, 'error': 'Erro ao atualizar gatilho'}), 500
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def routines_management(company_id: int):
    ...
    return render_template('process_routines.html', 
                          company=company, 
                          processes=processes,
    ...
    try:
        ...
        for row in cursor.fetchall():
            routines.append({
                ...
                'deadline_date': row[8],
                'process_code': row[9],
                'process_name': f"{row[9]} - {row[10]}" if row[9] and row[10] else row[10] or 'Não vinculado'
            })
...
@app.route("/api/companies/<int:company_id>/process-routines/<int:routine_id>", methods=['DELETE'])
...
@app.route("/api/companies/<int:company_id>/process-routines/<int:routine_id>", methods=['PUT'])
...
@app.route("/api/processes/<int:process_id>/routines-with-collaborators", methods=['GET'])
...
@app.route("/api/routines/<int:routine_id>/collaborators/<int:collaborator_id>", methods=['PUT'])
...
@app.route("/api/routines/<int:routine_id>/collaborators/<int:collaborator_id>", methods=['DELETE'])
...
def plan_dashboard(plan_id: str):
    ...
    section_keys = [
        'participants', 'interviews', 'vision', 'market', 'company',
        'alignment', 'directionals', 'alignments-found', 'misalignments-critical',
        'preliminary-analysis', 'workshop-final-analysis',
        'directionals-ai', 'directionals-consultant', 'directionals-approvals',
        'preliminary-analysis-okr', 'workshop-final-okr', 'okr-approvals',
        'preliminary-analysis-area-okr', 'area-okr-workshop', 'final-area-okr',
        'projects', 'projects-analysis'
    ]
    section_statuses = {key: db.get_section_status(int(plan_id), key) for key in section_keys}
    ...
    def make_section_leaf(node_id, label, section_name, meta=None):
        ...
        return make_leaf(node_id, label, value, has_record=section_has_record(section_name), meta=meta)

    def make_branch(node_id, label, children):
        children = children or []
        if children:
            avg_value = int(round(sum(child['value'] for child in children) / len(children)))
            ...
            make_branch(
                ...
                [
                    ...
                    make_leaf(
                        ...
                        meta=f"{len(company_financials)} linhas" if company_financials else None
                    )
                ]
            ),
            make_branch(
            ...
            make_branch(
                ...
                [
                    make_section_leaf(
                        ...
                        'participants',
                        meta=f"{len(participants)} cadastrados" if participants else None
                    )
                ]
            )
        ]
    ...
    drivers_branch = make_branch(
        ...
        [
            make_branch(
                ...
                'Diagnóstico inicial',
                [
                    make_section_leaf('drivers.diagnostico.entrevistas', 'Entrevistas com stakeholders', 'interviews'),
                    make_section_leaf('drivers.diagnostico.visao', 'Visão dos sócios', 'vision'),
                    make_section_leaf('drivers.diagnostico.mercado', 'Possibilidades do mercado', 'market'),
                    make_section_leaf('drivers.diagnostico.empresa', 'Possibilidades da empresa', 'company')
                ]
            ),
            make_branch(
                'drivers.analise',
                ...
                [
                    make_section_leaf('drivers.analise.alinhamentos', 'Alinhamentos encontrados', 'alignment'),
                    make_section_leaf('drivers.analise.catalogo', 'Catálogo de alinhamentos', 'alignments-found'),
                    make_section_leaf('drivers.analise.desalinhamentos', 'Desalinhamentos críticos', 'misalignments-critical'),
                    make_section_leaf('drivers.analise.preliminar', 'Análise preliminar', 'preliminary-analysis'),
                    make_section_leaf('drivers.analise.workshop', 'Workshop final de análise', 'workshop-final-analysis')
                ]
            ),
            make_branch(
                'drivers.formulacao',
                ...
                [
                    ...
                    make_section_leaf('drivers.formulacao.ia', 'Recomendações da IA', 'directionals-ai'),
                    make_section_leaf('drivers.formulacao.consultor', 'Parecer do consultor', 'directionals-consultant'),
                    make_section_leaf('drivers.formulacao.aprovacoes', 'Aprovações e comunicação', 'directionals-approvals')
                ]
            )
        ]
    )
    ...
    okr_global_branch = make_branch(
        ...
        [
            make_branch(
                ...
                'Ciclo de construção',
                [
                    make_section_leaf('okrs_global.ciclo.preliminar', 'Diagnóstico preliminar', 'preliminary-analysis-okr'),
                    make_section_leaf('okrs_global.ciclo.workshop', 'Workshop de construção', 'workshop-final-okr'),
                    make_section_leaf(
    ...
    okr_area_branch = make_branch(
        ...
        [
            make_branch(
                ...
                'Ciclo de construção',
                [
                    make_section_leaf('okrs_area.ciclo.preliminar', 'Diagnóstico preliminar', 'preliminary-analysis-area-okr'),
                    make_section_leaf('okrs_area.ciclo.workshop', 'Workshops por área', 'area-okr-workshop'),
                    make_section_leaf(
        ...
        [
            make_branch(
                ...
                [
                    ...
                    make_section_leaf('projects_progress.portfolio.analise', 'Análises e priorização', 'projects-analysis')
                ]
            )
        ]
    )
    ...
    for driver in drivers[:3]:
        ...
        summary = description[:100] + '...' if description and len(description) > 100 else description
        ...
        directionals.append({
            ...
            'status': status_raw.title() if isinstance(status_raw, str) else 'Em andamento'
        })
...
def plan_company(plan_id: str):
    ...
    company_section_open = company_section_status is None or company_section_status.get('status', 'open') == 'open'
    
    # Section status for analyses and summary
    analyses_section_status = db.get_section_status(int(plan_id), 'analyses')
    analyses_section_open = analyses_section_status is None or analyses_section_status.get('status', 'open') == 'open'
    
    summary_section_status = db.get_section_status(int(plan_id), 'summary')
    summary_section_open = summary_section_status is None or summary_section_status.get('status', 'open') == 'open'
    ...
    if company_data_row:
        ...
        for entry in financials_list:
            ...
            normalized_financials.append({
                'line': entry.get('line', ''),
                'revenue': float(revenue_decimal) if revenue_decimal is not None else 0,
                'margin': float(margin_decimal) if margin_decimal is not None else 0,
                'market': entry.get('market', '')
            })
        computed_revenue, computed_margin = _calculate_financial_totals(normalized_financials)
        ...
        total_revenue_value = computed_revenue if computed_revenue is not None else stored_revenue
        total_margin_value = computed_margin if computed_margin is not None else stored_margin
        if total_revenue_value is not None:
            total_revenue_value = total_revenue_value.quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
        if total_margin_value is not None:
            total_margin_value = total_margin_value.quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
        ...
        company_data = {
            ...
            'coverage_physical': company_data_row.get('coverage_physical') or 'regional',
            'coverage_online': company_data_row.get('coverage_online') or 'internet-nacional',
    ...
    coverage_physical_label = next((opt['label'] for opt in coverage_options if opt['id'] == company_data.get('coverage_physical')), 'Nacional')
    coverage_online_label = next((opt['label'] for opt in online_options if opt['id'] == company_data.get('coverage_online')), 'Internet nacional')
...
def plan_participants(plan_id: str):
    ...
    participants_section_open = participants_section_status is None or participants_section_status.get('status', 'open') == 'open'
...
def toggle_participant(plan_id: str, employee_id: int):
    ...
    try:
        ...
        if existing_participant:
            ...
            return jsonify({'success': True, 'action': 'removed', 'message': 'Participação removida com sucesso'})
        else:
            # Add participation
            participant_data = {
                'employee_id': employee_id,
            ...
            if participant_id:
                return jsonify({'success': True, 'action': 'added', 'message': 'Participante adicionado com sucesso'})
            else:
...
def plan_drivers(plan_id: str):
    ...
    interview_section_open = interview_section_status is None or interview_section_status.get('status', 'open') == 'open'
    ...
    vision_section_open = vision_section_status is None or vision_section_status.get('status', 'open') == 'open'
    ...
    market_section_open = market_section_status is None or market_section_status.get('status', 'open') == 'open'
    ...
    alignments_found_section_status = db.get_section_status(int(plan_id), 'alignments-found')
    alignments_found_section_open = alignments_found_section_status is None or alignments_found_section_status.get('status', 'open') == 'open'
    
    misalignments_critical_section_status = db.get_section_status(int(plan_id), 'misalignments-critical')
    misalignments_critical_section_open = misalignments_critical_section_status is None or misalignments_critical_section_status.get('status', 'open') == 'open'
    
    preliminary_analysis_section_status = db.get_section_status(int(plan_id), 'preliminary-analysis')
    preliminary_analysis_section_open = preliminary_analysis_section_status is None or preliminary_analysis_section_status.get('status', 'open') == 'open'
    
    workshop_final_analysis_section_status = db.get_section_status(int(plan_id), 'workshop-final-analysis')
    workshop_final_analysis_section_open = workshop_final_analysis_section_status is None or workshop_final_analysis_section_status.get('status', 'open') == 'open'
    ...
    directionals_section_open = directionals_section_status is None or directionals_section_status.get('status', 'open') == 'open'
    
    # Get new directionals sections status
    directionals_ai_section_status = db.get_section_status(int(plan_id), 'directionals-ai')
    directionals_ai_section_open = directionals_ai_section_status is None or directionals_ai_section_status.get('status', 'open') == 'open'
    
    directionals_consultant_section_status = db.get_section_status(int(plan_id), 'directionals-consultant')
    directionals_consultant_section_open = directionals_consultant_section_status is None or directionals_consultant_section_status.get('status', 'open') == 'open'
    
    
    directionals_approvals_section_status = db.get_section_status(int(plan_id), 'directionals-approvals')
    directionals_approvals_section_open = directionals_approvals_section_status is None or directionals_approvals_section_status.get('status', 'open') == 'open'
    ...
    if directionals_ai_section_status:
        ...
        directionals_ai_suggestions = directionals_ai_section_status.get('suggestions', '')
    
    if directionals_consultant_section_status:
        directionals_consultant_ai_analysis = directionals_consultant_section_status.get('ai_analysis', '')
        directionals_consultant_diagnosis = directionals_consultant_section_status.get('diagnosis', '')
        directionals_consultant_directionals = directionals_consultant_section_status.get('directionals', '')
    ...
    if misalignments_critical_section_status:
        misalignment_consultant_notes = misalignments_critical_section_status.get('notes', '')
    
    if workshop_final_analysis_section_status:
        workshop_consultant_notes = workshop_final_analysis_section_status.get('notes', '')
        # Try to get workshop adjustments from the adjustments field (JSON format)
        try:
            adjustments_data = workshop_final_analysis_section_status.get('adjustments', '')
            workshop_adjustments = json.loads(adjustments_data) if adjustments_data else []
    ...
    partners_options = ["Carlos Pina", "Marcos Fenecio", "Ana Souza", "Fabiano Ferreira"]
...
def plan_okr_global(plan_id: str):
    ...
    indicator_lookup = {key: value.get('label') or value.get('name') or '' for key, value in indicator_lookup_map.items()}

    preliminary_analysis_section_status = db.get_section_status(int(plan_id), 'preliminary-analysis-okr')
    preliminary_analysis_section_open = preliminary_analysis_section_status is None or preliminary_analysis_section_status.get('status', 'open') == 'open'

    workshop_final_section_status = db.get_section_status(int(plan_id), 'workshop-final-okr')
    workshop_final_section_open = workshop_final_section_status is None or workshop_final_section_status.get('status', 'open') == 'open'

    okr_approvals_section_status = db.get_section_status(int(plan_id), 'okr-approvals')
    okr_approvals_section_open = okr_approvals_section_status is None or okr_approvals_section_status.get('status', 'open') == 'open'
    ...
    if not workshop_raw and workshop_status and workshop_status.get('notes'):
        try:
            ...
            legacy_okrs = legacy_workshop.get('okrs') if isinstance(legacy_workshop, dict) else None
            if isinstance(legacy_okrs, list) and legacy_okrs:
                for legacy in legacy_okrs:
                    ...
                    okr_data = {
                        ...
                        'okr_type': okr_type,
                        'type_display': legacy.get('type_display') or _okr_type_display(okr_type),
                    ...
                    db.add_global_okr_record(int(plan_id), 'workshop', okr_data, legacy_key_results)
                legacy_workshop.pop('okrs', None)
                db.update_section_consultant_notes(int(plan_id), 'workshop-final-okr', json.dumps(legacy_workshop))
                workshop_raw = db.get_global_okr_records(int(plan_id), 'workshop')
                workshop_status = db.get_section_status(int(plan_id), 'workshop-final-okr')
        except Exception as exc:
    ...
    if not approvals_raw and approvals_status and approvals_status.get('notes'):
        try:
            ...
            legacy_okrs = legacy_approvals.get('okrs') if isinstance(legacy_approvals, dict) else None
            if isinstance(legacy_okrs, list) and legacy_okrs:
                for legacy in legacy_okrs:
                    okr_type = legacy.get('okr_type') or legacy.get('type') or ''
                    okr_data = {
                    ...
                    db.add_global_okr_record(int(plan_id), 'approval', okr_data, legacy_key_results)
                legacy_approvals.pop('okrs', None)
                db.update_section_consultant_notes(int(plan_id), 'okr-approvals', json.dumps(legacy_approvals))
                approvals_raw = db.get_global_okr_records(int(plan_id), 'approval')
                approvals_status = db.get_section_status(int(plan_id), 'okr-approvals')
        except Exception as exc:
    ...
    def _normalize_global_records(records):
        ...
        for record in records:
            ...
            item['type_display'] = item.get('type_display') or _okr_type_display(item.get('okr_type', ''))
            item['directional'] = str(item.get('directional')) if item.get('directional') is not None else ''
            ...
            for kr in item.get('key_results', []):
                normalized_kr = dict(kr)
                normalized_kr['id'] = str(normalized_kr.get('id')) if normalized_kr.get('id') is not None else None
                kr_owner_id = normalized_kr.get('owner_id')
                if kr_owner_id is not None and kr_owner_id != '':
                    kr_owner_str = str(kr_owner_id)
                    normalized_kr['owner_id'] = kr_owner_str
    ...
    workshop_discussions = workshop_discussions_data.get('content', '') if workshop_discussions_data else ''
    ...
    approval_discussions = approval_discussions_data.get('content', '') if approval_discussions_data else ''
...
def plan_okr_area(plan_id: str):
    ...
    participants_options, participant_name_lookup = _build_participant_context(int(plan_id))
    indicator_lookup_map = _build_indicator_lookup(company['id'])
    indicators = list(indicator_lookup_map.values())
    indicator_lookup = {key: value.get('label') or value.get('name') or '' for key, value in indicator_lookup_map.items()}
    
    # Get section statuses for all area OKR sections
    preliminary_analysis_status = db.get_section_status(int(plan_id), 'preliminary-analysis-area-okr')
    preliminary_analysis_section_open = preliminary_analysis_status is None or preliminary_analysis_status.get('status', 'open') == 'open'
    
    workshop_area_status = db.get_section_status(int(plan_id), 'area-okr-workshop')
    workshop_area_section_open = workshop_area_status is None or workshop_area_status.get('status', 'open') == 'open'
    
    final_area_status = db.get_section_status(int(plan_id), 'final-area-okr')
    final_area_section_open = final_area_status is None or final_area_status.get('status', 'open') == 'open'
                ...
                try:
                    ...
                    if isinstance(okr_data['deadline'], str):
                        if 'T' in okr_data['deadline'] or '+' in okr_data['deadline']:
                            okr_data['deadline'] = datetime.fromisoformat(okr_data['deadline'].replace('Z', '+00:00'))
                        else:
    ...
    area_workshop_discussions_data = db.get_workshop_discussions(int(plan_id), 'area-preliminary')
    area_workshop_discussions = area_workshop_discussions_data.get('content', '') if area_workshop_discussions_data else ''
    ...
    final_area_discussions_data = db.get_workshop_discussions(int(plan_id), 'area-final')
    final_area_discussions = final_area_discussions_data.get('content', '') if final_area_discussions_data else ''
    
    # Calculate summary for all OKRs
    all_area_okrs = workshop_area_okr_records + final_area_okr_records
    okr_summary = {
        "total": len(all_area_okrs),
        "comercial": len([okr for okr in all_area_okrs if okr.get('area') == 'comercial']),
        "operacional": len([okr for okr in all_area_okrs if okr.get('area') == 'operacional']),
        "administrativo": len([okr for okr in all_area_okrs if okr.get('area') == 'administrativo']),
        "estruturante": len([okr for okr in all_area_okrs if okr.get('type') == 'estruturante']),
        "aceleracao": len([okr for okr in all_area_okrs if okr.get('type') == 'aceleracao'])
    }
...
def plan_projects(plan_id: str):
    ...
    try:
        ...
        projects_section_open = projects_section_status is None or projects_section_status.get('status', 'open') == 'open'
        
        # Get projects analysis section status
        projects_analysis_section_status = db.get_section_status(int(plan_id), 'projects-analysis')
        projects_analysis_section_open = projects_analysis_section_status is None or projects_analysis_section_status.get('status', 'open') == 'open'
        ...
        project_summary = {
            "total": len(projects),
            "status": {
                "Em andamento": len([p for p in projects if p.get('status') == 'in_progress']),
                "Concluídos": len([p for p in projects if p.get('status') == 'completed']),
                "Planejados": len([p for p in projects if p.get('status') == 'planned'])
            }
        }
    ...
    try:
        ...
        while True:
            ...
            activity = {
                ...
                'observations': request.form.get(f'activity_observations_{activity_index}', '')
            }
...
def update_project_route(plan_id: str, project_id: int):
    ...
    try:
        ...
        status = existing_project.get('status', 'planned') if existing_project else 'planned'
        priority = request.form.get('priority', 'medium')
        owner = request.form.get('owner', '')
        start_date = request.form.get('start_date', '')
        end_date = request.form.get('end_date', '')
...
def plan_reports(plan_id: str):
        ...
        "catalog": [
            {
                ...
                "owner": driver['owner'],
                "updated_at": driver['updated_at'][:10] if driver['updated_at'] else driver['created_at'][:10]
            }
    ...
    directionals_consultant_status = db.get_section_status(int(plan_id), 'directionals-consultant')
    directionals_approvals_status = db.get_section_status(int(plan_id), 'directionals-approvals')
    
    sections_data['directionals'] = {
        'ai_analysis': directionals_status.get('notes', '') if directionals_status else '',
        'consultant_analysis': directionals_consultant_status.get('notes', '') if directionals_consultant_status else '',
        'approvals': directionals_approvals_status.get('notes', '') if directionals_approvals_status else ''
    }
    ...
    sections_data['okr_area'] = {
        'workshop': area_workshop_status.get('notes', '') if area_workshop_status else '',
    ...
    sections_data['projects'] = {
        'projects': projects,
        'analysis': projects_analysis_status.get('notes', '') if projects_analysis_status else ''
    }
...
def update_company_data(plan_id: str):
    ...
    total_revenue_decimal, total_margin_decimal = _calculate_financial_totals(financials)
    if total_revenue_decimal is not None:
        total_revenue_decimal = total_revenue_decimal.quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
    if total_margin_decimal is not None:
        total_margin_decimal = total_margin_decimal.quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
    ...
    data['financial_total_revenue'] = str(total_revenue_decimal) if total_revenue_decimal is not None else ''
    data['financial_total_margin'] = str(total_margin_decimal) if total_margin_decimal is not None else ''
...
def generate_company_ai_insights(plan_id: str):
    ...
    try:
        ...
        if not company_data:
            return jsonify({'success': False, 'error': 'Dados da empresa não encontrados'})
        ...
        if agent:
            ...
            if result.get('success'):
                output_field = agent.get('output_field', 'ai_insights') if isinstance(agent, dict) else 'ai_insights'
                ...
                try:
                    ...
                    existing_consultant = existing_analyses.get('consultant_analysis') or ''
                    if output_field == 'consultant_analysis':
                        db.update_company_analyses(int(plan_id), {
                    ...
                    pass
                return jsonify({'success': True, 'insights': insights_text, 'agent': result.get('agent_id')})
            else:
                # If agent execution failed, fallback to generic insights
                insights = ai_service.generate_insights(company_data, "Análise estratégica da empresa")
                if insights:
                    return jsonify({'success': True, 'insights': insights, 'fallback': True})
                return jsonify({'success': False, 'error': result.get('error') or 'Erro ao gerar insights da IA'})
        ...
        if insights:
            ...
            return jsonify({'success': False, 'error': 'Nenhum agente configurado e falha no gerador padrão'})
...
def update_company_section_status(plan_id: str):
    try:
        ...
        if status not in ('open', 'closed'):
            return jsonify({'success': False, 'error': 'Status inválido'}), 400
        ok = db.update_section_status(int(plan_id), 'company', status, closed_by=closed_by, notes=notes)
        if ok:
            # Retornar o status atual para UI
            current = db.get_section_status(int(plan_id), 'company') or {'status': status, 'closed_by': closed_by, 'notes': notes}
            return jsonify({'success': True, 'section': current})
        return jsonify({'success': False, 'error': 'Falha ao atualizar status da seção'}), 500
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def update_participant(plan_id: str, participant_id: str):
    ...
    if db.update_participant(participant_id, participant_data):
        ...
        return jsonify({'success': False, 'error': 'Erro ao atualizar participante'}), 500
...
@app.route("/plans/<plan_id>/participants/<participant_id>/send-message", methods=['POST'])
def send_participant_message(plan_id: str, participant_id: str):
    ...
    template = db.get_message_template(int(plan_id), message_type)
    if not template:
        return jsonify({'success': False, 'error': 'Template de mensagem não encontrado'}), 404
    ...
    processed_content = template['content'].replace('{{name}}', participant['name'] or '')
    ...
    processed_subject = template['subject'].replace('{{name}}', participant['name'] or '')
...
def update_interview(plan_id: str, interview_id: str):
    ...
    if db.update_interview(int(interview_id), interview_data):
        ...
        if request.is_json:
            return jsonify({'success': False, 'error': 'Erro ao atualizar entrevista'}), 500
        else:
...
def save_alignment_consultant_notes(plan_id: str):
    ...
    if db.update_section_consultant_notes(int(plan_id), 'alignments-found', consultant_notes):
...
def save_misalignment_consultant_notes(plan_id: str):
    ...
    if db.update_section_consultant_notes(int(plan_id), 'misalignments-critical', consultant_notes):
        flash('Parecer do consultor salvo com sucesso!', 'success')
    else:
        flash('Erro ao salvar parecer do consultor.', 'error')
...
def save_workshop_consultant_notes(plan_id: str):
    ...
    if db.update_section_consultant_notes(int(plan_id), 'workshop-final-analysis', consultant_notes):
        flash('Parecer do consultor salvo com sucesso!', 'success')
    else:
        flash('Erro ao salvar parecer do consultor.', 'error')
...
def add_workshop_adjustment(plan_id: str):
    ...
    try:
        adjustments = json.loads(section_status.get('adjustments', '[]')) if section_status else []
    except (json.JSONDecodeError, TypeError):
        adjustments = []
    ...
    if db.update_section_adjustments(int(plan_id), 'workshop-final-analysis', json.dumps(adjustments)):
...
def update_vision_record(plan_id: str, vision_id: str):
    ...
    if db.update_vision_record(int(vision_id), vision_data):
        ...
        if request.is_json:
            return jsonify({'success': False, 'error': 'Erro ao atualizar visão dos sócios'}), 500
        else:
...
def delete_vision_record(plan_id: str, vision_id: str):
    ...
    if db.delete_vision_record(int(vision_id)):
        ...
        return jsonify({'success': False, 'error': 'Erro ao excluir visão dos sócios'}), 500
...
def get_vision_record(plan_id: str, vision_id: str):
    ...
    if vision_record:
        ...
        return jsonify({'success': False, 'error': 'Visão dos sócios não encontrada'}), 404
...
def update_market_record(plan_id: str, market_id: str):
    ...
    if db.update_market_record(int(market_id), market_data):
        ...
        if request.is_json:
            return jsonify({'success': False, 'error': 'Erro ao atualizar possibilidades do mercado'}), 500
        else:
...
def delete_market_record(plan_id: str, market_id: str):
    ...
    if db.delete_market_record(int(market_id)):
        ...
        return jsonify({'success': False, 'error': 'Erro ao excluir possibilidades do mercado'}), 500
...
def get_market_record(plan_id: str, market_id: str):
    ...
    if market_record:
        ...
        return jsonify({'success': False, 'error': 'Possibilidades do mercado não encontradas'}), 404
...
def update_company_record(plan_id: str, company_id: str):
    ...
    if db.update_company_record(int(company_id), company_data):
        ...
        if request.is_json:
            return jsonify({'success': False, 'error': 'Erro ao atualizar possibilidades da empresa'}), 500
        else:
...
def delete_company_record(plan_id: str, company_id: str):
    ...
    if db.delete_company_record(int(company_id)):
        ...
        return jsonify({'success': False, 'error': 'Erro ao excluir possibilidades da empresa'}), 500
...
def get_company_record(plan_id: str, company_id: str):
    ...
    if company_record:
        ...
        return jsonify({'success': False, 'error': 'Possibilidades da empresa não encontradas'}), 404
...
def update_alignment_record(plan_id: str, alignment_id: str):
    ...
    if db.update_alignment_record(int(alignment_id), alignment_data):
        ...
        if request.is_json:
            return jsonify({'success': False, 'error': 'Erro ao atualizar alinhamento'}), 500
        else:
...
def update_misalignment_record(plan_id: str, misalignment_id: str):
    ...
    if db.update_misalignment_record(int(misalignment_id), misalignment_data):
        ...
        if request.is_json:
            return jsonify({'success': False, 'error': 'Erro ao atualizar desalinhamento'}), 500
        else:
            flash('Erro ao atualizar desalinhamento!', 'error')
            return redirect(url_for('plan_drivers', plan_id=plan_id))

@app.route("/plans/<plan_id>/misalignment-records/<misalignment_id>", methods=['DELETE'])
def delete_misalignment_record(plan_id: str, misalignment_id: str):
    ...
    if db.delete_misalignment_record(int(misalignment_id)):
        ...
        return jsonify({'success': False, 'error': 'Erro ao excluir desalinhamento'}), 500
...
def get_misalignment_record(plan_id: str, misalignment_id: str):
    ...
    if misalignment_record:
        ...
        return jsonify({'success': False, 'error': 'Desalinhamento não encontrado'}), 404
...
def update_directional_record(plan_id: str, directional_id: str):
    ...
    if db.update_directional_record(int(directional_id), directional_data):
        ...
        if request.is_json:
            return jsonify({'success': False, 'error': 'Erro ao atualizar direcionador'}), 500
        else:
...
def add_workshop_okr_record(plan_id: str):
    ...
    try:
        ...
        indicator_lookup_full = _build_indicator_lookup(company_id) if company_id else {}
        ...
        okr_data = {
            ...
            'type_display': request.form.get('okr_type', '').title() if request.form.get('okr_type') else '',
            'owner_id': owner_id,
        ...
        for kr in key_results:
            ...
            if indicator_id_val is not None:
                ...
                if indicator_data and not kr.get('indicator_label'):
                    kr['indicator_label'] = indicator_data.get('label') or indicator_data.get('name') or ''
        
        # Save to database
        okr_id = db.add_global_okr_record(int(plan_id), 'workshop', okr_data, key_results)
...
def add_okr_approval_record(plan_id: str):
    ...
    try:
        ...
        # Save to database
        okr_id = db.add_global_okr_record(int(plan_id), 'approval', okr_data, key_results)
...
@app.route("/plans/<plan_id>/okr-global/preliminary/<record_id>/delete", methods=['POST'])
...
def duplicate_okr_record(plan_id: str, type: str, record_id: str):
    ...
    try:
        ...
        okr_id = db.add_global_okr_record(int(plan_id), target_stage, okr_data, key_results)
...
def update_okr_preliminary_analysis_section_status(plan_id: str):
    ...
    try:
        ...
        success = db.update_section_status(int(plan_id), 'preliminary-analysis-okr', status, None)
...
def update_okr_workshop_final_section_status(plan_id: str):
    ...
    try:
        ...
        success = db.update_section_status(int(plan_id), 'workshop-final-okr', status, None)
...
def add_area_okr_record(plan_id: str):
    ...
    try:
        ...
        for kr in key_results_payload:
            ...
            kr_owner_name = kr.get('owner') or (participant_data.get('name') or '').strip()
            indicator_id_val = kr.get('indicator_id')
            ...
            indicator_label = kr.get('indicator_label') or indicator_data.get('label') or indicator_data.get('name') or ''
        ...
        try:
            ...
            """, (
                ...
                new_okr['owner'], new_okr['deadline'], new_okr['observations']
            ))
        ...
        if db.update_section_status(int(plan_id), 'area-okr-workshop', 'open', json.dumps(workshop_data)):
...
def add_final_area_okr_record(plan_id: str):
    ...
    try:
        ...
        if db.update_section_status(int(plan_id), 'final-area-okr', 'open', json.dumps(final_data)):
...
def update_area_preliminary_analysis_section_status(plan_id: str):
    ...
    try:
        ...
        if db.update_section_status(int(plan_id), 'preliminary-analysis-area-okr', status, notes):
            return jsonify({'success': True})
        else:
            return jsonify({'success': False, 'message': 'Erro ao atualizar status'}), 500
...
def get_agents():
    ...
    try:
        ...
        def _normalize(a):
            ...
            activation = agent.get('activation') if isinstance(agent.get('activation'), dict) else {
                'page': agent.get('page'),
                'page_label': None,
            ...
            def _label_section(value):
                ...
                return labels.get(value, value)
            activation['page_label'] = _label_page(activation.get('page')) if activation.get('page') else ''
            activation['section_label'] = _label_section(activation.get('section')) if activation.get('section') else ''

            # Input data
            input_data = agent.get('input_data') if isinstance(agent.get('input_data'), dict) else {
                'required': json.loads(agent.get('required_data', '[]')) if isinstance(agent.get('required_data'), str) else (agent.get('required_data') or []),
                'optional': json.loads(agent.get('optional_data', '[]')) if isinstance(agent.get('optional_data'), str) else (agent.get('optional_data') or [])
            }
            # Response format
            response_format = agent.get('response_format') if isinstance(agent.get('response_format'), dict) else {
                'type': agent.get('format_type', 'markdown'),
                'output_field': agent.get('output_field', 'ai_insights'),
                'template': agent.get('response_template', '')
            }
            # Advanced
            advanced_settings = agent.get('advanced_settings') if isinstance(agent.get('advanced_settings'), dict) else {
...
def create_agent():
    ...
    try:
        ...
        required_list = [s.strip() for s in str(data.get('required_data', '')).split(',') if s.strip()] if data.get('required_data') else []
        optional_list = [s.strip() for s in str(data.get('optional_data', '')).split(',') if s.strip()] if data.get('optional_data') else []
        db_payload = {
            'id': data['id'],
            'name': data['name'],
            'description': data.get('description', ''),
    ...
    try:
        ...
        if a:
            ...
            return jsonify({'success': True, 'agent': {
                ...
                'activation': {
                    'page': a.get('page') or (a.get('activation') or {}).get('page'),
                    'page_label': '',
                    'section': a.get('section') or (a.get('activation') or {}).get('section'),
                    'section_label': '',
                    'button_text': a.get('button_text') or (a.get('activation') or {}).get('button_text', '')
                },
                'input_data': {
                    'required': json.loads(a.get('required_data', '[]')) if isinstance(a.get('required_data'), str) else (a.get('required_data') or []),
                    'optional': json.loads(a.get('optional_data', '[]')) if isinstance(a.get('optional_data'), str) else (a.get('optional_data') or [])
                },
                'prompt_template': a.get('prompt_template', ''),
                'response_format': {
                    'type': a.get('format_type', 'markdown') if a.get('format_type') else (a.get('response_format') or {}).get('type', 'markdown'),
                    'output_field': a.get('output_field', 'ai_insights') if a.get('output_field') else (a.get('response_format') or {}).get('output_field', 'ai_insights'),
                    'template': a.get('response_template', '') if a.get('response_template') else (a.get('response_format') or {}).get('template', '')
                },
                'advanced_settings': {
...
def update_agent(agent_id):
    ...
    try:
        ...
        if success:
            return jsonify({'success': True, 'message': 'Agente atualizado com sucesso'})
        else:
...
def get_available_fields():
    ...
    try:
        ...
        if page == 'company' and section == 'vision':
            fields = [f for f in fields if f['value'] in ('mission','vision','values','ai_insights','consultant_analysis')]
        elif page == 'drivers' and section == 'market':
            fields = [f for f in fields if f['value'] in ('driver_topics','ai_insights','consultant_analysis')]
...
def api_company_portfolios(company_id: int):
    ...
    if not code:
        return jsonify({'success': False, 'message': 'Código do portfólio é obrigatório.'}), 400
    if not name:
        return jsonify({'success': False, 'message': 'Nome do portfólio é obrigatório.'}), 400
    ...
    try:
        ...
        cursor.execute(
            ...
            (
                ...
                code,
                name,
                int(responsible_id) if responsible_id and str(responsible_id).strip() else None,
                notes
            )
        )
...
@app.route("/api/companies/<int:company_id>/portfolios/<int:portfolio_id>", methods=['PUT', 'DELETE'])
    ...
    if request.method == 'DELETE':
        try:
            ...
            if not _portfolio_exists(cursor, company_id, portfolio_id):
                conn.close()
                return jsonify({'success': False, 'message': 'Portfólio não encontrado.'}), 404
...
def _ensure_activity_codes(activities: List[Dict[str, Any]], project_code: Optional[str]) -> List[Dict[str, Any]]:
...
def _compute_project_status_from_activities(activities: List[Dict[str, Any]]) -> str:
    ...
    progress_markers = {'in_progress', 'on_time', 'progress', 'executing', 'executando', 'em andamento'}
...
def _serialize_company_project(row) -> Dict[str, Any]:
    ...
    return {
        ...
        'okr_reference': row['okr_reference'] if 'okr_reference' in row.keys() else None,
        'indicator_reference': row['indicator_reference'] if 'indicator_reference' in row.keys() else None,
        ...
        'code': project_code,
        'code_sequence': row['code_sequence'] if 'code_sequence' in row.keys() else None,
        'created_at': row['created_at'],
        'updated_at': row['updated_at']
    }
        ...
        if plan_id:
            ...
            if plan_row and plan_row['company_id'] == company_id:
                ...
                if portfolio_row and portfolio_row['company_id'] == company_id:
                    ...
                    conn.close()
                    return jsonify({'success': False, 'message': 'Planejamento ou portfólio inválido para esta empresa.'}), 400
        ...
        if plan_id:
            ...
            if plan_row and plan_row['company_id'] == company_id:
                ...
                if portfolio_row and portfolio_row['company_id'] == company_id:
                    # É um portfolio GRV válido
                    plan_id_value = int(plan_id)
                    if not plan_type:  # Se não foi fornecido no payload, definir como GRV
                        plan_type = 'GRV'
                else:
                    conn.close()
                    return jsonify({'success': False, 'message': 'Planejamento ou portfólio inválido para esta empresa.'}), 400
...
@app.route("/api/companies/<int:company_id>/projects/<int:project_id>/activities", methods=['GET', 'POST'])
def api_project_activities(company_id: int, project_id: int):
    ...
    if request.method == 'GET':
        try:
            ...
            activities, changed, _ = normalize_project_activities(activities, project_code)
    ...
    if not what:
        return jsonify({'success': False, 'message': 'Descrição da atividade é obrigatória.'}), 400
    ...
    try:
        ...
        created_activity = next((act for act in activities if act.get('id') == new_id), new_activity)
...
@app.route("/api/companies/<int:company_id>/projects/<int:project_id>/activities/<int:activity_id>", methods=['PUT', 'DELETE'])
...
@app.route("/api/companies/<int:company_id>/projects/<int:project_id>/activities/<int:activity_id>/stage", methods=['PATCH'])
        ...
        for activity in activities:
            if activity.get('id') == activity_id:
                ...
                if stage == 'completed':
                    ...
                    if not activity.get('completion_date') and 'completion_date' not in payload:
                        from datetime import datetime
                        activity['completion_date'] = datetime.now().strftime('%Y-%m-%d')
                else:
...
@app.route("/api/companies/<int:company_id>/projects/<int:project_id>/activities/<int:activity_id>/transfer", methods=['POST'])
def api_transfer_activity(company_id: int, project_id: int, activity_id: int):
    ...
    if not target_project_id:
        return jsonify({'success': False, 'message': 'ID do projeto de destino é obrigatório.'}), 400
    
    if target_project_id == project_id:
        return jsonify({'success': False, 'message': 'Não é possível transferir para o mesmo projeto.'}), 400
    ...
    try:
        ...
        if not source_row:
            conn.close()
            return jsonify({'success': False, 'message': 'Projeto de origem não encontrado.'}), 404
        ...
        if not target_row:
            conn.close()
            return jsonify({'success': False, 'message': 'Projeto de destino não encontrado.'}), 404
        ...
        target_activities, _, target_max_sequence = normalize_project_activities(target_activities, target_code)
        ...
        activity_code = f"{target_code}.{next_sequence:02d}" if target_code and next_sequence else None

        if not activity_code:
            fallback_code, _ = _generate_activity_code(cursor, company_id, target_project_id)
        ...
        source_activities, _, _ = normalize_project_activities(source_activities, source_code)
        target_activities, _, _ = normalize_project_activities(target_activities, target_code)
...
@app.route("/api/companies/<int:company_id>/projects/<int:project_id>/info", methods=['GET'])
