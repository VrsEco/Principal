...
def test_authentication_register_endpoint():
    ...
    try:
        ...
        resp = session.post(
            AUTH_REGISTER_URL, json=new_user_payload, headers=headers, timeout=30
        )
        assert (
            resp.status_code == 200
        ), f"Expected 200 status for user creation, got {resp.status_code}"
        resp_json = resp.json()
        assert resp_json.get(
            "success", True
        ), "User creation response does not indicate success"
        ...
        for invalid_data in invalid_payloads:
            resp_invalid = session.post(
                AUTH_REGISTER_URL, json=invalid_data, headers=headers, timeout=30
            )
            assert (
        ...
        resp_no_auth = session_no_auth.post(
            AUTH_REGISTER_URL, json=new_user_payload, headers=headers, timeout=30
        )
        # According to PRD, 403 is "Access denied" - no valid session means no permission
        assert (
            resp_no_auth.status_code == 403
        ), f"Expected 403 status for unauthorized access, got {resp_no_auth.status_code}"
