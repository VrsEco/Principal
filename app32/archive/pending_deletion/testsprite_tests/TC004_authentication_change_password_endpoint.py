...
def test_authentication_change_password_endpoint():
    ...
    try:
        login_resp = session.post(
            BASE_URL + AUTH_LOGIN_ENDPOINT, json=login_payload, timeout=timeout
        )
        assert (
        ...
        ), f"Change password success failed with status {resp.status_code}"

        # 3. Test validation failure: missing new_password
        invalid_payload_1 = {"current_password": "newStrongP@ssw0rd"}
        resp = session.post(
            BASE_URL + CHANGE_PASSWORD_ENDPOINT, json=invalid_payload_1, timeout=timeout
        )
        assert (
            resp.status_code == 400
        ), f"Expected 400 for missing new_password, got {resp.status_code}"
        ...
        resp = session.post(
            BASE_URL + CHANGE_PASSWORD_ENDPOINT, json=invalid_payload_2, timeout=timeout
        )
        assert (
            resp.status_code == 400
        ), f"Expected 400 for invalid new_password, got {resp.status_code}"
        ...
        try:
            ...
            resp = session.post(
                BASE_URL + CHANGE_PASSWORD_ENDPOINT, json=reset_payload, timeout=timeout
            )
    ...
    no_auth_payload = {"current_password": "123456", "new_password": "anotherNewP@ss1"}
    no_auth_resp = requests.post(
        BASE_URL + CHANGE_PASSWORD_ENDPOINT, json=no_auth_payload, timeout=timeout
    )
    assert (
        no_auth_resp.status_code == 401
    ), f"Expected 401 unauthorized but got {no_auth_resp.status_code}"
