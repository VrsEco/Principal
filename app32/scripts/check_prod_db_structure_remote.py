...
def run_remote():
    ssh = connect_ssh()
    try:
        local_script = Path(__file__).parent / "check_prod_db_structure.py"
        with open(local_script, "r", encoding="utf-8") as f:
            content = f.read()
            
        import base64
        b64_content = base64.b64encode(content.encode()).decode()
        remote_file = f"{APP_DIR}/check_prod_db_structure_remote.py"
        ...
        with open("scripts/check_db_structure_output.txt", "w", encoding="utf-8") as f:
            f.write("--- STDOUT ---\n")
            f.write(out)
            f.write("\n--- STDERR ---\n")
            f.write(err)
        
        print("Done. Output written to scripts/check_db_structure_output.txt")
