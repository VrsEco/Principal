...
def calculate_deadline(
    deadline_value: int, deadline_unit: str, scheduled_date: datetime
) -> datetime:
...
def process_trigger(
    routine: Dict[str, Any], trigger: Dict[str, Any], current_time: datetime
) -> bool:
    """
    ...
    if should_fire:
        ...
        deadline_date = calculate_deadline(
            trigger["deadline_value"], trigger["deadline_unit"], current_time
...
def process_routines():
    ...
    print("=" * 80)
    print(
        f"Iniciando processamento de rotinas - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
    print("=" * 80)
