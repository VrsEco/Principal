param(
    ...
    [string]$Surface = "ops",
    [string]$ServerName = "app32-laboratorio-ops",
...
Write-Step "Instalação do Codex para o laboratório Versus"
...
$harnessPath = Join-Path $WorkspaceRoot "app32\.ai\codex-squad-engenharia-laboratorio.md"
...
$serverConfig = @{
    ...
    metadata = @{
        profile = $Profile
        profile_label = "Squad de Engenharia"
        surface = $Surface
...
if (-not (Test-Path -LiteralPath $harnessPath)) {
    throw "Harness do Codex não encontrado em: $harnessPath"
}
...
Write-Host "1. Abra o Codex no workspace correto."
Write-Host "2. Use o harness: $harnessPath"
Write-Host "3. Valide a sequência de startup do Squad de Engenharia."
