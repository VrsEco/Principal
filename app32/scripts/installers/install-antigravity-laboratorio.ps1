param(
    ...
    [string]$Token,
    [int]$CompanyId = 10,
    [string]$Profile = "squad_versus",
    [string]$Surface = "admin",
    [string]$ServerName = "app32-laboratorio-admin",
...
Write-Step "Instalação do Antigravity para o laboratório Versus"
...
$harnessPath = Join-Path $WorkspaceRoot "app32\.ai\antigravity-squad-versus-laboratorio.md"
...
if (-not (Test-Path -LiteralPath $harnessPath)) {
    throw "Harness do Antigravity não encontrado em: $harnessPath"
}
...
Write-Host "1. Abra o Antigravity no contexto correto."
Write-Host "2. Use o harness: $harnessPath"
Write-Host "3. Valide a sequência de startup do Squad Versus."
