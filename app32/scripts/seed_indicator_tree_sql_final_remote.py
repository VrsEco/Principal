...
def run_remote():
    ssh = connect_ssh()
    try:
        local_script = Path(__file__).parent / "seed_indicator_tree_sql_final.py"
        with open(local_script, "r", encoding="utf-8") as f:
            content = f.read()
            
        import base64
        b64_content = base64.b64encode(content.encode()).decode()
        remote_file = f"{APP_DIR}/seed_indicator_tree_sql_final_remote.py"
        ...
        with open("scripts/seed_tree_sql_final_output.txt", "w", encoding="utf-8") as f:
            f.write("--- STDOUT ---\n")
            f.write(out)
            f.write("\n--- STDERR ---\n")
            f.write(err)
        
        print("Done. Output written to scripts/seed_tree_sql_final_output.txt")
