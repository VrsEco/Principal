...
def run_remote():
    ssh = connect_ssh()
    try:
        local_script = Path(__file__).parent / "migrate_indicators_v2_logic.py"
        ...
        remote_file = f"{APP_DIR}/migrate_indicators_v2_remote.py"
        ...
        with open("scripts/migration_indicators_output.txt", "w", encoding="utf-8") as f:
            f.write("--- STDOUT ---\n")
            f.write(out)
            f.write("\n--- STDERR ---\n")
            f.write(err)
        
        print("Done. Output written to scripts/migration_indicators_output.txt")
