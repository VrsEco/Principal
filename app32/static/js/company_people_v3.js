(() => {
  ...
  function buildOrgChartSvg() {
    ...
    const connectors = rendered.filter(entry => entry.parentId != null && byRoleId.has(Number(entry.parentId))).map(entry => { const parent = byRoleId.get(Number(entry.parentId)); const fromX = treeOriginX + parent.x + parent.width / 2; const fromY = treeOriginY + parent.y + parent.height; const toX = treeOriginX + entry.x + entry.width / 2; const toY = treeOriginY + entry.y; const middleY = Math.round((fromY + toY) / 2); return `<path d="M${Math.round(fromX)} ${Math.round(fromY)}V${middleY}H${Math.round(toX)}V${Math.round(toY)}" fill="none" stroke="#8fa6bf" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>`; }).join('');
