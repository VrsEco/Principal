...
def test_process_step_put_accepts_video_upload(monkeypatch):
    ...
    monkeypatch.setattr(process_resource, 'save_file', lambda file, subfolder='': saved_paths.append((file.filename, subfolder)) or f'{subfolder}/{file.filename}')
    ...
    assert status == 200
    assert response['video_path'] == 'pop/video/passo.mp4'
