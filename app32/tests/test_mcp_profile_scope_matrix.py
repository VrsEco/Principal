...
def test_finance_contract_uses_privileged_surface_and_gates_mutations():
    ...
    for operation in create_or_update:
        ...
        assert operation.surface == "mcp_finance"
...
def test_customer_finance_contract_uses_privileged_surface_and_gate():
    customer_allowed = []
    ...
    finance_allowed = [item for item in customer_allowed if item[0] == "finance"]
    non_finance_allowed = [item for item in customer_allowed if item[0] != "finance"]

    assert finance_allowed
    assert all(surface in {"mcp_finance", "mcp_analytics"} for _, _, surface in finance_allowed)
    assert all(surface == "mcp_user" for _, _, surface in non_finance_allowed)
    assert all(action in {"read", "list", "analyze"} for _, action, _ in non_finance_allowed)

    finance_contract = APP32_CRUD_CONTRACTS_MANIFEST.get_domain("finance")
    assert finance_contract is not None
    customer_finance_mutations = [
        operation
        for operation in finance_contract.operations
        if "cliente" in operation.allowed_roles and operation.action in {"create", "update"}
    ]
    assert customer_finance_mutations
    assert all(operation.surface == "mcp_finance" for operation in customer_finance_mutations)
    assert all(operation.human_gate_required for operation in customer_finance_mutations)
