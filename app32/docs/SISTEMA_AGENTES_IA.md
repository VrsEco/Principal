# 🤖 Sistema de Agentes IA - OpenAI

**Versão:** APP32 (v1.3)
**Data:** 14/02/2026
**Plataforma:** OpenAI (GPT-4o + Embeddings) + Internal Python Services
...
2. [OpenAI Platform (O Cérebro)](#openai-platform)
...
Criar um **ecossistema de agentes IA especializados** usando OpenAI para:
...
> ⚠️ **ATENÇÃO:** A arquitetura detalhada segue estritamente a governança definida em `docs/governance/AGENT_ARCHITECTURE.md` (Modelo Híbrido Externalizado). O diagrama abaixo é uma representação da implementação específica usando Google Cloud.

```
┌─────────────────────────────────────────────────────────────────┐
│                    GOOGLE CLOUD PLATFORM (O Cérebro)            │
│                                                                 │
│  ├─ Agentes Externos (Cloud Run / OpenAI / n8n)                  │
│  ├─ Agente PEV (Planejamento)                                   │
│  ├─ Agente Processos (Otimização)                               │
│  ├─ Agente Rotina (Follow-up)                                   │
│  ├─ Agente Performance (Analytics)                              │
│  └─ LLMs (GPT-4o)                                               │
│                                                                 │
│           ▲ (API REST / Inbound)                                │
│           │                                                     │
│           ▼ (Webhooks / Outbound)                               │
│                                                                 │
│                    APP32 (O Corpo & Interface)                  │
│  ├─ Agentes Internos (Cadastro, Diagnóstico) 🆕                 │
...
**Status:** 🆕 ✅ Implementado como **Agente Interno** (MVP) em `services/cadastro_agent_service.py`
...
# agents/base_agent.py
    def __init__(self, agent_id, model="gpt-4o"):
...
class PEVAgent(BaseAgent):
    ...
    def extract_recommendations(self, analysis_text):
        ...
        # Usa OpenAI para extrair recomendações estruturadas
        prompt = f"""
...
- [ ] Agente RH
- [ ] Agente Comercial
- [x] Agente Cadastro & Configuração (Internal Agent MVP)
...
**Status:** 🚀 Em Implementação (Fase 1 + Agentes Internos)
**Próximo Passo:** Consolidar Agentes Externos (PEV, Processos) via n8n/Vertex AI
