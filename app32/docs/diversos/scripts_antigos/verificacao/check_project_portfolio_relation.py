...
def check_project_portfolio_relation():
    ...
    try:
        ...
        cursor.execute(
            "SELECT name FROM pragma_table_info('company_projects') WHERE name='plan_id'"
        )
        ...
        if project_count > 0 and plan_id_exists:
            ...
            cursor.execute(
                "SELECT id, code, title, plan_id FROM company_projects WHERE plan_id IS NOT NULL LIMIT 5"
            )
            projects_with_plan = cursor.fetchall()
            print(f"Projetos com plan_id (primeiros 5):")
            for proj in projects_with_plan:
                print(
                    f"  - ID: {proj['id']}, Code: {proj['code']}, Title: {proj['title']}, Plan ID: {proj['plan_id']}"
                )
            ...
            for pp in portfolio_projects:
                print(
                    f"  - Portfólio: {pp['portfolio_name']} (ID: {pp['portfolio_id']}) - Projetos: {pp['project_count']}"
                )
