...
def check_meetings():
    ...
    try:
        ...
        cursor.execute(
            "SELECT id, company_id, title, scheduled_date, status FROM meetings ORDER BY company_id, id"
        )
        ...
        for meeting in all_meetings:
            meeting_dict = dict(meeting)
            print(
                f"  ID: {meeting_dict.get('id')}, Company: {meeting_dict.get('company_id')}, Titulo: {meeting_dict.get('title')}"
            )
        ...
        for col in columns:
            col_dict = dict(col)
            print(
                f"  {col_dict.get('column_name')}: {col_dict.get('data_type')} {'NULL' if col_dict.get('is_nullable') == 'YES' else 'NOT NULL'}"
            )
