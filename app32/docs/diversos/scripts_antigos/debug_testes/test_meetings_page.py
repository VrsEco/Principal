...
def test_page():
    ...
    try:
        ...
        if (
            "Teste Fabiano Reunião 02" in response.text
            or "Teste Fabiano Reuni" in response.text
        ):
        ...
        if (
            "Nenhuma reunião cadastrada" in response.text
            or "Nenhuma reuni" in response.text
        ):
