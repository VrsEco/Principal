...
def test_configuration():
    ...
    try:
        ...
        if config.SQLALCHEMY_DATABASE_URI:
            db_type = (
                "PostgreSQL"
                if "postgresql" in config.SQLALCHEMY_DATABASE_URI.lower()
                else "SQLite"
            )
    ...
    for file in essential_files:
        ...
        results.append(
            test_result(
                f"Arquivo {file}", exists, "Existe" if exists else "Não encontrado"
            )
        )
...
def test_database():
    ...
    try:
        ...
        results.append(
            test_result("config_database", True, f"Módulo carregado ({db_type})")
        )
    except Exception as e:
    ...
    for model_name in models_to_test:
        try:
            ...
            if model_class:
                ...
                results.append(
                    test_result(f"Model {model_name}", False, "Classe não encontrada")
                )
        except ImportError:
            results.append(
                test_result(f"Model {model_name}", False, "Módulo não encontrado")
            )
        except Exception as e:
    ...
    try:
        ...
        temp_app.config["SQLALCHEMY_DATABASE_URI"] = (
...
def test_flask_app():
    ...
    try:
        ...
        if spec and spec.loader:
            ...
            results.append(
                test_result("Import app_pev", False, "Arquivo não encontrado")
            )
    except Exception as e:
        ...
        if found_blueprints:
            results.append(
                test_result(
                    "Blueprints",
                    True,
                    f"{len(found_blueprints)}/{len(blueprints)} encontrados",
                )
            )
    ...
    try:
        ...
        for route, name in main_routes:
            if f"@app.route('{route}'" in content or f'@app.route("{route}"' in content:
                ...
                results.append(
                    test_result(f"Rota {name}", False, f"{route} não encontrada")
                )
...
def test_modules():
    ...
    for module_path, module_name in modules_to_test:
        try:
            ...
            if blueprint:
                results.append(
                    test_result(f"Módulo {module_name}", True, "Blueprint encontrado")
                )
            else:
                results.append(
                    test_result(
                        f"Módulo {module_name}", False, "Blueprint não encontrado"
                    )
                )
            ...
            results.append(
                test_result(
                    f"Módulo {module_name}", False, f"Import error: {str(e)[:50]}"
                )
            )
        except Exception as e:
...
def test_services():
    ...
    try:
        ...
        results.append(
            test_result("Route Audit Service", False, f"Erro: {str(e)[:50]}")
        )
...
def test_apis():
    ...
    if os.path.exists("api"):
        ...
        for api_file in api_files:
            ...
            results.append(
                test_result(
                    f"API {api_file}", exists, "Existe" if exists else "Não encontrado"
                )
            )
    else:
    ...
    try:
        ...
        for route_pattern, name in api_routes:
            if route_pattern in content:
                results.append(
                    test_result(f"Rota API {name}", True, f"{route_pattern}")
                )
            else:
                results.append(
                    test_result(
                        f"Rota API {name}", False, f"{route_pattern} não encontrada"
                    )
                )
...
def test_templates_static():
    ...
    for dir_path, name in dirs_to_check:
        ...
        results.append(
            test_result(
                f"Diretório {name}", exists, "Existe" if exists else "Não encontrado"
            )
        )
    ...
    for route, name in routes_to_test:
        try:
            response = requests.get(
                f"{base_url}{route}", timeout=timeout, allow_redirects=False
            )
            status = response.status_code

            if status in [200, 302, 401]:
                results.append(
                    test_result(f"Servidor {name}", True, f"Status {status}")
                )
            else:
                results.append(
                    test_result(f"Servidor {name}", False, f"Status {status}")
                )

        except requests.exceptions.ConnectionError:
            results.append(
                test_result(f"Servidor {name}", False, "Servidor não está rodando")
            )
        except Exception as e:
            results.append(test_result(f"Servidor {name}", False, str(e)[:50]))
...
def test_dependencies():
    ...
    for module_name, display_name in optional_deps:
        try:
            ...
            results.append(
                test_result(
                    display_name, False, "Não instalado (opcional)", warning=True
                )
            )
