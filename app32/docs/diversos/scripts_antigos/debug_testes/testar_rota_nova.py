...
try:
    response = requests.get(
        "http://localhost:5002/configs/system/audit", timeout=5, allow_redirects=False
    )
    ...
    if response.status_code == 404:
        ...
        print(
            "3. Verifique a mensagem: 'Sistema de auditoria de rotas integrado com sucesso!'"
        )

    elif response.status_code == 302 or response.status_code == 401:
        print(
            "[OK] Rota existe! Redirecionando para login (status {})".format(
