...
| Financial | Páginas, catálogos, títulos, relatórios/exportações | Cadastros financeiros + título: criar conta bancária, plano de conta, centro de resultado, favorecido, forma financeira; editar cadastros; criar título manual; validar detalhe; editar; cancelar; excluir título e cadastros. Falta ampliar para borderô, transferência e baixa/estorno assistido. | criar/editar/cancelar/excluir | DELETE lógico com payload sem marcador persistente e auditoria `residue_total=0` | Parcial implementado |
...
| Integrations/MCP/Sapiens/IA | Catálogo, requests, página API/MCP, MCP health, concorrência | Solicitação fake de integração: validar catálogo, renderizar API/MCP, checar `/mcp/healthz`, criar request, listar e excluir via teardown tenant-safe; quando houver card de backlog derivado, excluí-lo junto. Falta ampliar para providers reais sandbox e tool request/workflow request com rollback. | GET/GET/GET/POST/GET/DELETE | hard delete de `integration_requests` e do card derivado marcado `integration_request_id=<id>`; auditoria `residue_total=0` | Parcial implementado |
