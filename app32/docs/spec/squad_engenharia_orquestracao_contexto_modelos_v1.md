...
`Sapiens Engenharia` é **local-only**. O manifesto do SE-COORD é consumido apenas pelo ambiente local; MCP remoto não lista Engenharia na ativação e `describe_app32_squad_runtime_tool(runtime_profile='engineering')` retorna `squad_runtime_local_only`. Não criar conexão MCP, perfil remoto, rota ou catálogo paralelo para esta função. A triagem local informa campos aceitos, complexidade/risco/economia como saída, as quatro estratégias de contexto e `model_selection=manual_outside_assessment`; não executa especialista, lê estado operacional ou concede acesso.

Checklist da entrega **[Sapiens Engenharia — triagem de complexidade e economia de contexto]**: [x] manifesto único em service; [x] ativação e bootstrap MCP canônicos; [x] interação guiada no tool canônico; [x] modo conversacional efêmero no runtime cliente; [x] regressão protegida; [ ] materialização/fechamento remoto do card, bloqueados sem contorno pela aprovação externa já registrada.

No local, `EngineeringTaskRouterService` e `EngineeringTokenEconomyService` recebem a tarefa diretamente e devolvem resposta deliberadamente estreita: intenção, complexidade, risco, pendências e `token_economy`; não expõem seleção de especialista, não aceitam campos de autoridade ou contexto empresarial e não fazem leitura de dados. Tentativa de ativar Engenharia pelo MCP remoto falha fechada.

### Modo conversacional local

`Squad Engenharia On` é um contrato de conversa **local**, não uma mutação de banco ou chamada MCP. O runtime local mantém estado efêmero da conversa atual e, em cada solicitação técnica seguinte, chama os services locais de triagem antes de ampliar contexto. `Squad Engenharia Off`, troca de modo, nova conversa ou mudança de identidade descartam o estado. Não há reaproveitamento entre `thread_id`, empresa, usuário ou runtime; o APP32 não persiste esse estado para simular sessão autenticada. O Codex/local segue essa convenção nesta conversa; o instalador de comandos MCP remoto recusa Engenharia.
