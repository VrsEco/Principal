...
def get_publication_suggestions(snapshot: MacroProcessSipocSnapshot) -> list[str]:
    """Retorna recomendações SIPOC que não impedem a publicação."""
    errors: list[str] = []
    if not _normalize_text(snapshot.start_boundary):
        errors.append("Sugestão: informe o início do macroprocesso para deixar clara a fronteira da cadeia.")
    if not _normalize_text(snapshot.end_boundary):
        errors.append("Sugestão: informe o fim do macroprocesso para deixar clara a fronteira da cadeia.")
    ...
    if lane_counts.get("process", 0) < 3:
        errors.append("Sugestão metodológica: geralmente 3 ou mais processos filhos ou grandes etapas tornam a cadeia mais legível.")
    if lane_counts.get("process", 0) > 7:
        errors.append("Sugestão metodológica: considere agrupar os processos filhos ou grandes etapas para facilitar a leitura do SIPOC.")
