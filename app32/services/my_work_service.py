    ...
    try:
        ...
        for row in rows:
            row_dict = dict(row) if isinstance(row, dict) else {
                ...
                "company_id": row[3], "company_name": row[4], "company_code": row[5]
            }
            ...
            processes.append(
                {
                    "id": process_id,
                    "name": row_dict.get("name") or "Processo sem nome",
                    "code": code,
                    "company_id": row_dict.get("company_id"),
                    "company_name": row_dict.get("company_name"),
                    "company_code": row_dict.get("company_code"),
...
def _fetch_v2_project_rows(
    ...
    if employee_ids:
        clause, clause_params = _build_in_clause(employee_ids, "v2_target_ids")
        if clause:
            params.update(clause_params)
            conditions.append(f"""(
                pt.employee_id IN ({clause}) 
                OR pt.id IN (SELECT activity_id FROM project_activity_collaborators WHERE is_deleted = FALSE AND employee_id IN ({clause}))
            )""")
            filters_applied = True

    if company_ids:
        clause, clause_params = _build_in_clause(company_ids, "v2_company_ids")
        if clause:
            params.update(clause_params)
            conditions.append(f"p.company_id IN ({clause})")
            filters_applied = True

    if project_ids:
        clause, clause_params = _build_in_clause(project_ids, "v2_project_ids")
        if clause:
            params.update(clause_params)
            conditions.append(f"pt.project_id IN ({clause})")
            filters_applied = True
    ...
    try:
        query = f"""
            SELECT
                pt.id AS activity_id,
                'PT-' || pt.id AS activity_code,
                pt.what AS activity_title,
                pt.how AS activity_description,
                pt.status AS activity_status,
                pt.stage AS activity_stage,
                pt.priority AS activity_priority,
                pt.due_date AS activity_deadline,
                pt.estimated_hours,
                pt.worked_hours,
                pt.amount,
                NULL AS metadata,
                pt.project_id,
                pt.employee_id AS responsible_id,
                NULL AS executor_id,
                ...
                p.notes AS project_description,
            ...
            LEFT JOIN employees resp ON resp.id = pt.employee_id
            LEFT JOIN plans pl ON pl.id = p.plan_id
            LEFT JOIN companies c ON c.id = p.company_id
            WHERE {" AND ".join(conditions)}
            ORDER BY pt.due_date NULLS LAST, pt.updated_at DESC
        """
...
def _fetch_normalized_project_rows(
    ...
    # New V2 logic directly fetching from new schema tables
    v2_activities = _fetch_v2_project_rows(
        cursor, employee_ids=employee_ids, company_ids=company_ids, project_ids=project_ids
    )
    ...
    combined_v1 = table_activities if not legacy_activities else _merge_activity_sources(table_activities, legacy_activities)
    return combined_v1 + v2_activities
...
def _collect_my_activities(
    ...
    """Retorna atividades do colaborador (ou lista de colaboradores) sem aplicar filtros."""
    
    # Normalizar lista de IDs
    target_ids = []
    if employee_ids:
        target_ids.extend(employee_ids)
    if employee_id and employee_id not in target_ids:
        target_ids.append(employee_id)
        
    if not target_ids:
        return []

    project_rows = _fetch_projects_for_employee(cursor, target_ids)
    process_rows = _fetch_processes_for_employee(cursor, target_ids)

    # Identificar qual employee_id correta para serialização de cada linha
    # Quando o usuário tem múltiplos IDs (ex: admin multi-tenancy), precisamos
    # passar o ID específico que está na linha para que o serializer identifique
    # como 'Responsável' ou 'Executor' em vez de 'Equipe'.
    viewer_id = target_ids[0] if target_ids else 0

    activities = []
    for row in project_rows:
        data = dict(row)
        rid = data.get("responsible_id")
        xid = data.get("executor_id")
        # Se a linha está vinculada a algum dos meus IDs, usa esse ID para o serializer
        # garantir que saia como "Minha" atividade.
        row_viewer_id = viewer_id
        if rid in target_ids:
            row_viewer_id = rid
        elif xid in target_ids:
            row_viewer_id = xid
        activities.append(_serialize_project_activity(row, row_viewer_id, member_ids=target_ids))

    for row in process_rows:
        # Para processos, verificamos se algum colaborador da instância é um dos meus IDs
        data = dict(row)
        collabs = _parse_collaborators(data.get("collaborators"))
        collab_ids = {c.get("id") for c in collabs if c.get("id")}
        
        row_viewer_id = viewer_id
        my_collab_id = next((cid for cid in target_ids if cid in collab_ids), None)
        if my_collab_id:
            row_viewer_id = my_collab_id
            
        activities.append(_serialize_process_activity(row, row_viewer_id, member_ids=target_ids))
...
def _get_company_activities_unrestricted(
    cursor,
    company_ids: List[int],
    filters: Optional[Dict] = None,
) -> List[Dict]:
    """Busca atividades de múltiplas empresas sem restrição de employee_id.

    Usado para usuários admin/client que não têm employee vinculado mas
    precisam visualizar todas as atividades de suas empresas.
    """
    filters = filters or {}
    ...
    for company_id in company_ids:
        ...
        # employee_id=0 indica "sem responsável específico" — serialization
        # ainda incluirá dados de responsável/executor dos dados da linha
        activities.extend(
            _serialize_project_activity(row, employee_id=0) for row in project_rows
        )
        activities.extend(
            _serialize_process_activity(row, employee_id=0) for row in process_rows
        )
...
def _fetch_projects_for_employee(cursor, employee_ids: List[int]):
    """Busca projetos onde os colaboradores são responsáveis ou executores."""
    if not employee_ids:
        return []
        
    rows = _fetch_normalized_project_rows(cursor, employee_ids)
    if rows:
        return rows

    # Fallback para queries manuais (se tabela normalizada falhar)
    placeholders = ",".join(["%s"] * len(employee_ids))
...
def _fetch_projects_for_members(cursor, member_ids: Sequence[int]):
    ...
    placeholders = ",".join(["%s"] * len(member_tuple))
...
def _fetch_processes_for_employee(cursor, employee_ids: List[int]):
    """Busca processos onde os colaboradores estão designados."""
    if not employee_ids:
        return []
        
    rows = _fetch_normalized_process_rows(cursor, employee_ids=employee_ids)
    if rows:
        return rows

    # Fallback complexo para tabelas não normalizadas:
    # Teríamos que buscar company_ids de todos os employees e filtrar
    # Para simplificar e como o sistema deve ter tabelas normalizadas ou ser migrado,
    # vamos implementar um loop simples se ocorrer fallback, 
    # ou uma query mais complexa se necessário.
    
    # Estratégia de Fallback: buscar empresas de todos esses employees
    company_ids = _fetch_companies_for_members(cursor, employee_ids)
    if not company_ids:
        return []

    # Buscar processos dessas empresas
    legacy_rows = _fetch_company_processes(cursor, company_ids[0]) # Limitado a uma empresa no fallback simples
    # Se houver múltiplas empresas no fallback legado, precisaríamos iterar. 
    # Mas assumindo que _fetch_normalized funciona na v2, ok.
    if len(company_ids) > 1:
        # Se for multi-empresa no legado, iteramos aqui (ineficiente mas fallback)
        all_rows = []
        for cid in company_ids:
            c_rows = _fetch_company_processes(cursor, cid)
            all_rows.extend(c_rows)
        legacy_rows = all_rows
        
    target_set = set(employee_ids)
    return [row for row in legacy_rows if _is_employee_in_process(row, target_set)]
...
def _serialize_project_activity(
    ...
    return {
        ...
        "viewer_is_directly_assigned": assignment.get("type") in ("responsible", "executor"),
        "company_id": data.get("company_id"),
        "company_name": data.get("company_name"),
        "plan_id": data.get("plan_id"),
        "plan_name": data.get("plan_name"),
        "completed_date": _date_to_iso(_coerce_date(data.get("completion_date"))),
...
def _serialize_process_activity(
    ...
    # Extrair dono e responsável para facilitar filtros e exibição
    owner_name = next((c.get('name') for c in collaborators if (c.get('role') or '').lower() == 'owner'), '')
    responsible_name = next((c.get('name') for c in collaborators if (c.get('role') or '').lower() == 'responsible'), '')
    if not responsible_name:
        # Fallback para executor se não houver responsável explícito
        responsible_name = next((c.get('name') for c in collaborators if (c.get('role') or '').lower() == 'executor'), '')
    ...
    return {
        ...
        "progress_percent": _calc_progress(estimated_hours, worked_hours),
        "assignment": assignment,
        "viewer_is_directly_assigned": assignment.get("type") == "assigned",
        "owner_name": owner_name,
        "responsible_name": responsible_name,
        "company_id": data.get("company_id"),
        "company_name": data.get("company_name"),
        "completed_date": _date_to_iso(_coerce_date(data.get("actual_end_date") or data.get("completed_at"))),
        "created_at": _datetime_to_iso(created_dt),
        "updated_at": _datetime_to_iso(updated_dt),
        "deadline_sort_key": _deadline_sort_key(deadline),
        "created_sort_key": _datetime_sort_key(created_dt),
...
def _apply_filters(activities: List[Dict], filters: Dict) -> List[Dict]:
    ...
    project_selection = filters.get("project_selection")
    process_selection = filters.get("process_selection")

    filtered = activities

    if project_selection == 'none':
        filtered = [a for a in filtered if a.get("type") != "project"]
    if process_selection == 'none':
        filtered = [a for a in filtered if a.get("type") != "process"]
    ...
    if project_ids and project_selection != 'none':
        filtered = [
            activity
            for activity in filtered
            if activity.get("type") != "project"
    ...
    if process_ids and process_selection != 'none':
        filtered = [
            activity
            for activity in filtered
