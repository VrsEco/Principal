...
EXECUTION_TEMPLATES: list[dict[str, Any]] = [
    ...
    {
        "key": "generic_webhook_outbound",
        "label": "Webhook / API Externa",
        "execution_mode": "api_task",
        "scope": "task",
        "summary": "Dispara webhook/API externa com payload mapeado a partir do runtime do processo.",
        "objective": "Enviar evento do processo para aplicação externa com contrato HTTP controlado.",
        "rest_config_json": {
            "connection_key": "external_webhook",
            "method": "POST",
            "path": "/process-events",
            "timeout_seconds": 20,
            "retry_policy": "default",
            "request_mapping": {
                "company_id": "{company_id}",
                "process_instance_id": "{instance_id}",
                "activity_id": "{current_bpmn_element_id}",
            },
            "response_schema": {"type": "object"},
        },
    },
    {
        "key": "email_notification_api",
        "label": "Notificação por E-mail",
        "execution_mode": "api_task",
        "scope": "task",
        "summary": "Dispara comunicação outbound por e-mail a partir do runtime do processo.",
        "objective": "Enviar e-mail operacional estruturado quando a atividade for de comunicação/notificação.",
        "rest_config_json": {
            "connection_key": "service_email",
            "method": "POST",
            "path": "/send",
            "timeout_seconds": 15,
            "retry_policy": "default",
            "request_mapping": {
                "company_id": "{company_id}",
                "process_instance_id": "{instance_id}",
                "subject": "{instance_title}",
            },
            "response_schema": {"type": "object"},
        },
    },
    {
        "key": "whatsapp_notification_api",
        "label": "Notificação por WhatsApp",
        "execution_mode": "api_task",
        "scope": "task",
        "summary": "Dispara mensagem operacional por WhatsApp via integração externa configurada.",
        "objective": "Enviar alerta operacional via WhatsApp quando a atividade for de cobrança, aviso ou lembrete.",
        "rest_config_json": {
            "connection_key": "service_whatsapp",
            "method": "POST",
            "path": "/messages/send",
            "timeout_seconds": 15,
            "retry_policy": "default",
