...
def financial_report_filters_page(report_slug: str):
    ...
    report = None
    if report_definition["code"] in {"schedule_report", "bank_statement", "income_statement", "income_statement_2"}:
...
def financial_report_view_page(report_slug: str):
    if str(report_slug or '').strip().lower() in {'agendamento', 'extrato-bancario', 'demonstrativo-resultados', 'demonstrativo-resultados-02'}:
