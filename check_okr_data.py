...
cursor.execute("SELECT plan_id, section_name, notes FROM plan_sections WHERE section_name LIKE '%okr%'")
records = cursor.fetchall()

print("Registros de OKR encontrados:")
for record in records:
    plan_id, section_name, notes = record
    print(f"Plan {plan_id}, Section: {section_name}, Notes length: {len(notes) if notes else 0}")
