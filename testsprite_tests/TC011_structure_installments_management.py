...
def test_structure_installments_management():
    ...
    plan_id = create_test_plan(
        session, company_id, "Test Plan Installments", "2025-01-01", "2025-12-31"
    )
    ...
    assert response.status_code in [
        200,
        201,
    ], f"Falha ao criar estrutura: {response.status_code} - {response.text}"
    ...
    if delete_response.status_code == 404:
        pytest.fail(
            f"❌ Endpoint DELETE /installments não existe! Status: {delete_response.status_code}"
        )

    assert (
        delete_response.status_code == 200
    ), f"Falha ao deletar parcelas: {delete_response.status_code} - {delete_response.text}"
    delete_result = delete_response.json()
    assert delete_result.get(
        "success"
    ), f"Erro ao deletar parcelas: {delete_result.get('error')}"
    ...
    for inst in installments:
        ...
        if create_response.status_code == 404:
            pytest.fail(
                f"❌ Endpoint POST /installments não existe! Status: {create_response.status_code}"
            )

        assert create_response.status_code in [
            200,
            201,
        ], f"Falha ao criar parcela: {create_response.status_code} - {create_response.text}"
        create_result = create_response.json()
        assert create_result.get(
            "success"
        ), f"Erro ao criar parcela: {create_result.get('error')}"
    ...
    assert len(created_installment_ids) == len(
        installments
    ), f"Esperado {len(installments)} parcelas, criadas {len(created_installment_ids)}"

    # 6. Verificar se parcelas foram salvas (buscar estrutura completa)
    get_structure_url = (
        f"{BASE_URL}/pev/api/implantacao/{plan_id}/structures/{structure_id}"
    )
    get_response = session.get(get_structure_url, timeout=TIMEOUT)
    assert (
        get_response.status_code == 200
    ), f"Falha ao buscar estrutura: {get_response.status_code}"

    structure_result = get_response.json()
    assert structure_result.get(
        "success"
    ), f"Erro ao buscar estrutura: {structure_result.get('error')}"

    structure_data = structure_result.get("data", {})
    saved_installments = structure_data.get("installments", [])
    assert len(saved_installments) == len(
        installments
    ), f"Esperado {len(installments)} parcelas salvas, encontradas {len(saved_installments)}"
    ...
    assert (
        delete_response.status_code == 200
    ), f"Falha ao deletar parcelas na edição: {delete_response.status_code}"
    ...
    for inst in edited_installments:
        ...
        assert create_response.status_code in [
            200,
            201,
        ], f"Falha ao criar parcela editada: {create_response.status_code}"
    ...
    assert len(saved_installments) == len(
        edited_installments
    ), f"Esperado {len(edited_installments)} parcelas após edição, encontradas {len(saved_installments)}"

    print(
        f"✅ Edição de parcelas funcionou corretamente - {len(saved_installments)} parcelas salvas"
    )
