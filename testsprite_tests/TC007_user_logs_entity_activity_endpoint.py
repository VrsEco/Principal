...
def test_logs_entity_activity_endpoint():
    ...
    try:
        ...
        assert response.status_code in (
            ...
            404,
            500,
        ), "Unexpected status code for valid entity test"
        if response.status_code == 200:
            content_type = response.headers.get("Content-Type", "").lower()
            assert (
                "application/json" in content_type
            ), "Response Content-Type should include 'application/json' when status 200"
            try:
            ...
            assert any(
                key in json_resp for key in ["logs", "message", "success"]
            ), "Expected keys missing in response"
            ...
            assert (
                False
            ), f"Unhandled status code {response.status_code} for valid entity test"
    ...
    try:
        ...
        assert response.status_code in (
            404,
            500,
        ), "Expected 404 or 500 status for invalid entity test"
        if response.status_code == 404:
            ...
            assert (
                False
            ), f"Unhandled status code {response.status_code} for invalid entity test"
    except requests.RequestException as e:
