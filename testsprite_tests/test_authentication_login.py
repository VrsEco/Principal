...
def test_authentication_login_endpoint():
    ...
    invalid_payloads = [
        ...
        ({"email": AUTH_CREDENTIALS["username"]}, 400),  # missing password - Bad Request
        ({"password": AUTH_CREDENTIALS["password"]}, 400),  # missing email - Bad Request
        ({"email": "", "password": ""}, 400),  # empty strings - Bad Request
        ({"email": "not_an_email", "password": "pass"}, 401),  # invalid email format - treated as invalid credentials
    ]
    for payload, expected_status in invalid_payloads:
        try:
            ...
            assert response.status_code == expected_status, f"Expected {expected_status} for invalid payload {payload}, got {response.status_code}"
        except Exception as e:
