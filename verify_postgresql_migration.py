...
def compare_table_data(sqlite_conn, postgres_conn, table_name):
    ...
    try:
        ...
        if sqlite_count != postgres_count:
            status = "⚠️"
            issues.append(f"Contagem diferente: SQLite={sqlite_count}, PostgreSQL={postgres_count}")
        
        if len(sqlite_columns) != len(postgres_columns):
            status = "⚠️"
            issues.append(f"Número de colunas diferente: SQLite={len(sqlite_columns)}, PostgreSQL={len(postgres_columns)}")
    ...
    for check in critical_checks:
        try:
            ...
            if count >= check['min_count']:
                print(f"  ✅ {check['name']}: {count} registros ({check['description']})")
            else:
                print(f"  ❌ {check['name']}: apenas {count} registros ({check['description']})")
                all_ok = False
...
def generate_report(comparison_results):
    ...
    if error_tables > 0:
        ...
        for result in comparison_results:
            if result['status'] == '❌':
                print(f"  - {result['table']}: {result.get('error', 'Erro desconhecido')}")
