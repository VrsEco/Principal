...
def test_meeting_json():
    ...
    try:
        ...
        if meeting:
            ...
            print(
                f"guests (tipo: {type(meeting.get('guests'))}): {meeting.get('guests')}"
            )
            print(
                f"agenda (tipo: {type(meeting.get('agenda'))}): {meeting.get('agenda')}"
            )
            print(
                f"participants (tipo: {type(meeting.get('participants'))}): {meeting.get('participants')}"
            )
            print(
                f"discussions (tipo: {type(meeting.get('discussions'))}): {meeting.get('discussions')}"
            )
            print(
                f"activities (tipo: {type(meeting.get('activities'))}): {meeting.get('activities')}"
            )
            ...
            if meeting.get("guests") is not None:
                ...
                print(
                    "AVISO: Os campos JSON estao None (pode ser normal se nao houver dados)"
                )
        else:
