...
class IntegrationConfigurator:
    ...
    def save_config(self):
        ...
        with open(self.env_file, "w") as f:
            ...
            f.write("# Database\n")
            f.write(
                f"DATABASE_URL={self.config.get('DATABASE_URL', 'sqlite:///pevapp22.db')}\n"
            )
            f.write(
                f"DEV_DATABASE_URL={self.config.get('DEV_DATABASE_URL', 'sqlite:///pevapp22_dev.db')}\n\n"
            )

            # Security
            f.write("# Security\n")
            f.write(
                f"SECRET_KEY={self.config.get('SECRET_KEY', 'dev-secret-key-change-in-production')}\n"
            )
            ...
            f.write(
                f"MAIL_DEFAULT_SENDER={self.config.get('MAIL_DEFAULT_SENDER', '')}\n\n"
            )
            ...
            f.write(
                f"WHATSAPP_PROVIDER={self.config.get('WHATSAPP_PROVIDER', 'z-api')}\n"
            )
            f.write(f"WHATSAPP_API_KEY={self.config.get('WHATSAPP_API_KEY', '')}\n")
            f.write(
                f"WHATSAPP_WEBHOOK_URL={self.config.get('WHATSAPP_WEBHOOK_URL', '')}\n"
            )
            f.write(
                f"WHATSAPP_INSTANCE_ID={self.config.get('WHATSAPP_INSTANCE_ID', '')}\n\n"
            )

            # Redis
            f.write("# Redis\n")
            f.write(
                f"REDIS_URL={self.config.get('REDIS_URL', 'redis://localhost:6379/0')}\n\n"
            )
    ...
    def configure_ai(self):
        ...
        if choice == "1":
            ...
            print("✅ Local AI analysis configured")
    ...
    def configure_email(self):
        ...
        if choice == "1":
            ...
            if webhook_url:
                ...
                print("✅ Email webhook configured")
            else:
                print("❌ Webhook URL required")

        elif choice == "3":
            self.config["EMAIL_PROVIDER"] = "local"
            print("✅ Local email simulation configured")
    ...
    def configure_whatsapp(self):
        ...
        if choice == "1":
            ...
            print("✅ Local WhatsApp simulation configured")
    ...
    def configure_database(self):
        ...
        if choice == "1":
            ...
            if all([database, username, password]):
                ...
                print("❌ PostgreSQL configuration incomplete")
    ...
    def run(self):
        """Run configuration"""
        print("🚀 PEVAPP22 Integration Configuration")
        print("=" * 50)
