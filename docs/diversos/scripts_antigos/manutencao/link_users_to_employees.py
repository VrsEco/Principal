...
def link_users_to_employees():
    ...
    if len(users) == 0:
        print("⚠️  Nenhum usuário encontrado. Execute primeiro o script de criação de usuários.")
        return
        ...
        try:
            ...
            if current_user_id is not None:
                if current_user_id == user.id:
                    print(f"   ✅ Já vinculado: Employee #{employee_id} -> User #{user.id}")
                    already_linked_count += 1
                else:
                    print(f"   ⚠️  Employee #{employee_id} já vinculado a outro user (#{current_user_id})")
                continue
            ...
            conn.commit()
            linked_count += 1
            print(f"   ✅ VINCULADO: Employee #{employee_id} ({employee_name}) -> User #{user.id} ({user.name})")
    ...
    if linked_count > 0:
        ...
        print("   Certifique-se de que existem employees com emails correspondentes aos users.")
