...
class LogService:
    ...
    def create_log(
        ...
        try:
            ...
            try:
                old_values_str = (
                    json.dumps(old_values, ensure_ascii=True) if old_values else None
                )
                new_values_str = (
                    json.dumps(new_values, ensure_ascii=True) if new_values else None
                )
    ...
    def log_view(
        ...
        return LogService.create_log(
            ...
            entity_name=entity_name,
            description=description
            or f"Visualiza̤̣o de {entity_type}"
    ...
    def get_logs(
        ...
        return (
            query.order_by(UserLog.created_at.desc()).offset(offset).limit(limit).all()
        )
    ...
    def get_log_stats(company_id=None, start_date=None, end_date=None):
        ...
        # Group by action
        actions = (
            db.session.query(UserLog.action, db.func.count(UserLog.id))
            .filter(UserLog.id.in_(query.with_entities(UserLog.id)))
        ...
        entities = (
            db.session.query(UserLog.entity_type, db.func.count(UserLog.id))
            .filter(UserLog.id.in_(query.with_entities(UserLog.id)))
            .group_by(UserLog.entity_type)
            .all()
        )

        # Group by user
        users = (
            db.session.query(UserLog.user_name, db.func.count(UserLog.id))
            .filter(UserLog.id.in_(query.with_entities(UserLog.id)))
            .group_by(UserLog.user_name)
            .order_by(db.func.count(UserLog.id).desc())
            .limit(10)
            .all()
        )
