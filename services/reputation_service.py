...
class ReputationService:
    ...
    def search_company_reputation(self, company_name: str, cnpj: str = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            results['recommendations'] = self._generate_recommendations(results['sources'])
    ...
    def _generate_summary(self, sources: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if sources.get('news_search', {}).get('status') == 'success':
                ...
                positive_news = len([a for a in articles if a.get('sentiment') == 'positive'])
                if positive_news > 0:
                    summary['positive_indicators'].append(f'{positive_news} notícias positivas recentes')
            ...
            if sources.get('social_media', {}).get('status') == 'success':
                ...
                if total_followers > 1000:
                    summary['positive_indicators'].append(f'Presença forte em redes sociais ({total_followers} seguidores)')
    ...
    def _generate_recommendations(self, sources: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            if sources.get('google_search', {}).get('status') == 'success':
                ...
                if len(results) < 5:
                    recommendations.append('Considere investir em SEO para melhorar a visibilidade online')
            
            if sources.get('social_media', {}).get('status') == 'success':
                platforms = sources['social_media'].get('platforms', {})
                if len(platforms) < 3:
                    recommendations.append('Expanda a presença em diferentes redes sociais')
            
            if sources.get('news_search', {}).get('status') == 'success':
                articles = sources['news_search'].get('articles', [])
                if len(articles) < 3:
                    recommendations.append('Invista em estratégias de comunicação e relacionamento com a mídia')
