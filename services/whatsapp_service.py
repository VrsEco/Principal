...
class WhatsAppService:
    ...
    def _send_zapi_message(self, phone_number: str, message: str, 
                          media_url: Optional[str] = None) -> bool:
        """Send message using Z-API"""
        if not all([self.api_key, self.instance_id]):
            print("Z-API configuration incomplete")
            return False
    ...
    def _send_twilio_message(self, phone_number: str, message: str, 
                            media_url: Optional[str] = None) -> bool:
    ...
    def _send_webhook_message(self, phone_number: str, message: str, 
                             media_url: Optional[str] = None) -> bool:
    ...
    def _send_local_message(self, phone_number: str, message: str, 
                           media_url: Optional[str] = None) -> bool:
        """Simulate WhatsApp message sending locally"""
        print(f"LOCAL WHATSAPP SIMULATION:")
    ...
    def test_connection(self) -> Dict[str, Any]:
        """
        Testa a conexão com o provedor de WhatsApp configurado
    ...
    def _test_zapi_connection(self) -> Dict[str, Any]:
        ...
        if not all([self.api_key, self.instance_id]):
            return {
                'success': False,
                'error': 'Configuração Z-API incompleta (API key e Instance ID necessários)',
                'provider': 'z-api'
            }
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get('status') == 'connected':
                    ...
                    return {
                        'success': False,
                        'error': f'Z-API não conectado: {result.get("status", "unknown")}',
                        'provider': 'z-api'
                    }
            else:
                return {
    ...
    def _test_twilio_connection(self) -> Dict[str, Any]:
        ...
        if not all([account_sid, auth_token]):
            return {
                'success': False,
                'error': 'Configuração Twilio incompleta (Account SID e Auth Token necessários)',
                'provider': 'twilio'
            }
    ...
    def _test_webhook_connection(self) -> Dict[str, Any]:
        ...
        try:
            response = requests.post(
                self.webhook_url,
                json={'test': True, 'message': 'Teste de conexão WhatsApp'},
                timeout=10
            )
    ...
    def send_welcome_message(self, phone_number: str, participant_name: str, plan_name: str) -> bool:
        """Send welcome WhatsApp message to participant"""
        message = f"""
        🎉 *Bem-vindo ao Planejamento Estratégico!*

        Olá *{participant_name}*!

        Você foi convidado a participar do plano estratégico *"{plan_name}"*.

        Sua contribuição é muito importante para o sucesso desta iniciativa! 🚀

        Em breve você receberá mais informações sobre como participar.
    ...
    def send_reminder_message(self, phone_number: str, participant_name: str, 
                             task_description: str, deadline: str) -> bool:
        ...
        Você tem uma tarefa pendente no planejamento estratégico:

        📋 *Tarefa:* {task_description}
        📅 *Prazo:* {deadline}

        Por favor, não esqueça de concluir esta atividade.
