...
class RouteAuditService:
    ...
    def discover_all_routes(app: Optional[Flask] = None) -> List[Dict[str, Any]]:
        ...
        for rule in app.url_map.iter_rules():
            ...
            try:
                ...
                if view_func:
                    ...
                    try:
                        ...
                        if (
                            "log_service" in source
                            or "log_create" in source
                            or "log_update" in source
                            or "log_delete" in source
                        ):
            ...
            route_info["needs_logging"] = (
                route_info["is_crud"]
                and not route_info["endpoint"].startswith("logs.")
                and not route_info["endpoint"].startswith("static")
                and route_info["endpoint"] not in ["login", "auth.login", "auth.logout"]
            )
    ...
    def get_routes_without_logging(app: Optional[Flask] = None) -> List[Dict[str, Any]]:
        ...
        routes_without_logging = [
            ...
            if route["needs_logging"]
            and not route["has_auto_log"]
            and not route["has_manual_log"]
        ]
    ...
    def get_audit_summary(app: Optional[Flask] = None) -> Dict[str, Any]:
        ...
        routes_with_logging = [
            r for r in crud_routes if r["has_auto_log"] or r["has_manual_log"]
        ]
        routes_without_logging = [
            r
            for r in crud_routes
            if r["needs_logging"] and not r["has_auto_log"] and not r["has_manual_log"]
        ]

        # Agrupar por blueprint
        by_blueprint = defaultdict(
            lambda: {"total": 0, "with_logging": 0, "without_logging": 0}
        )
        ...
        return {
            ...
            "routes_needing_logging": len(
                [r for r in crud_routes if r["needs_logging"]]
            ),
            "coverage_percentage": coverage_percentage,
            "by_blueprint": dict(by_blueprint),
            "by_entity": dict(by_entity),
            "critical_missing": routes_without_logging[
                :10
            ],  # Top 10 rotas crÃ­ticas sem log
        }

    @staticmethod
    def get_route_details(
        endpoint: str, app: Optional[Flask] = None
