...
class DatabaseRestore:
    ...
    def list_backups(self, db_type='all'):
        ...
        backups = {
            'database': sorted(
                self.backup_dir.glob('backup_*sql') if db_type == 'postgresql'
                else self.backup_dir.glob('backup_*.db') if db_type == 'sqlite'
                else list(self.backup_dir.glob('backup_*sql')) + list(self.backup_dir.glob('backup_*.db')),
    ...
    def restore_uploads(self, backup_file):
        ...
        if uploads_dir.exists():
            ...
            backup_current = uploads_dir.parent / f'uploads_backup_antes_restore_{timestamp}'
