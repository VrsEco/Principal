...
class OKRGlobal(db.Model):
    ...
    # Relationships
    key_results = db.relationship(
        "KeyResult", backref="okr_global", lazy="dynamic", cascade="all, delete-orphan"
    )
...
class KeyResult(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    okr_global_id = db.Column(
        db.Integer, db.ForeignKey("okrs_global.id"), nullable=False
    )
