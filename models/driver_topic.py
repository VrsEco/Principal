...
class DriverTopic(db.Model):
    ...
    )  # interviews, vision-socios, market, company
    ...
    def to_dict(self):
        ...
        return {
            ...
            "maturity_updated_at": self.maturity_updated_at.isoformat()
            if self.maturity_updated_at
            else None,
...
class DriverItem(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    driver_topic_id = db.Column(
        db.Integer, db.ForeignKey("driver_topics.id"), nullable=False
    )
