...
class Plan(db.Model):
    ...
    participants = db.relationship(
        "Participant", backref="plan", lazy="dynamic", cascade="all, delete-orphan"
    )
    company_data = db.relationship(
        "CompanyData", backref="plan", uselist=False, cascade="all, delete-orphan"
    )
    driver_topics = db.relationship(
        "DriverTopic", backref="plan", lazy="dynamic", cascade="all, delete-orphan"
    )
    okrs_global = db.relationship(
        "OKRGlobal", backref="plan", lazy="dynamic", cascade="all, delete-orphan"
    )
    okrs_area = db.relationship(
        "OKRArea", backref="plan", lazy="dynamic", cascade="all, delete-orphan"
    )
    projects = db.relationship(
        "Project", backref="plan", lazy="dynamic", cascade="all, delete-orphan"
    )
