...
class Product(db.Model):
    ...
    market_size_monthly_revenue = db.Column(
    ...
    def to_dict(self):
        ...
        return {
            ...
            # Custos Variáveis
            "variable_costs_percent": float(self.variable_costs_percent)
            if self.variable_costs_percent
            else 0.00,
            "variable_costs_value": float(self.variable_costs_value)
            if self.variable_costs_value
            else 0.00,
            "variable_costs_notes": self.variable_costs_notes,
            # Despesas Variáveis
            "variable_expenses_percent": float(self.variable_expenses_percent)
            if self.variable_expenses_percent
            else 0.00,
            "variable_expenses_value": float(self.variable_expenses_value)
            if self.variable_expenses_value
            else 0.00,
            "variable_expenses_notes": self.variable_expenses_notes,
            # Margem de Contribuição
            "unit_contribution_margin_percent": float(
                self.unit_contribution_margin_percent
            )
            if self.unit_contribution_margin_percent
            else 0.00,
            "unit_contribution_margin_value": float(self.unit_contribution_margin_value)
            if self.unit_contribution_margin_value
            else 0.00,
            "unit_contribution_margin_notes": self.unit_contribution_margin_notes,
            # Tamanho do Mercado
            "market_size_monthly_units": float(self.market_size_monthly_units)
            if self.market_size_monthly_units
            else 0.00,
            "market_size_monthly_revenue": float(self.market_size_monthly_revenue)
            if self.market_size_monthly_revenue
            else 0.00,
            "market_size_notes": self.market_size_notes,
            # Market Share
            "market_share_goal_monthly_units": float(
                self.market_share_goal_monthly_units
            )
            if self.market_share_goal_monthly_units
            else 0.00,
            "market_share_goal_percent": float(self.market_share_goal_percent)
            if self.market_share_goal_percent
            else 0.00,
    ...
    def from_dict(data, product=None):
        ...
        if "sale_price" in data:
            product.sale_price = (
                float(data["sale_price"]) if data["sale_price"] else 0.00
            )
        ...
        if "variable_costs_percent" in data:
            product.variable_costs_percent = (
                float(data["variable_costs_percent"])
                if data["variable_costs_percent"]
                else 0.00
            )
        if "variable_costs_value" in data:
            product.variable_costs_value = (
                float(data["variable_costs_value"])
                if data["variable_costs_value"]
                else 0.00
            )
        ...
        if "variable_expenses_percent" in data:
            product.variable_expenses_percent = (
                float(data["variable_expenses_percent"])
                if data["variable_expenses_percent"]
                else 0.00
            )
        if "variable_expenses_value" in data:
            product.variable_expenses_value = (
                float(data["variable_expenses_value"])
                if data["variable_expenses_value"]
                else 0.00
            )
        ...
        if "unit_contribution_margin_notes" in data:
            product.unit_contribution_margin_notes = data[
                "unit_contribution_margin_notes"
            ]

        # Tamanho do Mercado
        if "market_size_monthly_units" in data:
            product.market_size_monthly_units = (
                float(data["market_size_monthly_units"])
                if data["market_size_monthly_units"]
                else 0.00
            )
        ...
        if "market_share_goal_monthly_units" in data:
            product.market_share_goal_monthly_units = (
                float(data["market_share_goal_monthly_units"])
                if data["market_share_goal_monthly_units"]
                else 0.00
            )
        if "market_share_goal_percent" in data:
            product.market_share_goal_percent = (
                float(data["market_share_goal_percent"])
                if data["market_share_goal_percent"]
                else 0.00
            )
