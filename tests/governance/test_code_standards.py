...
class TestForbiddenPatterns:
        ...
        for py_file in get_python_files():
            ...
            for i, line in enumerate(lines, 1):
                ...
                if (
                    ...
                    and "#" not in line[: line.find("print")]
                ):
                    violations.append(f"{py_file}:{i}")

        assert (
            not violations
        ), f"print() statements encontrados (usar logger):\n" + "\n".join(violations)
    ...
    def test_no_bare_except(self):
        ...
        assert (
            not violations
        ), f"Bare except encontrados (especificar exceção):\n" + "\n".join(violations)
...
class TestCodingStandards:
    ...
    def test_imports_organization(self):
        ...
        assert (
            not violations
        ), f"import * encontrado (usar imports explícitos):\n" + "\n".join(violations)
    ...
    def test_no_commented_code(self):
        ...
        if violations:
            pytest.skip(
                f"Código comentado encontrado (revisar se é código morto):\n"
                + "\n".join(violations)
            )
                    ...
                    if isinstance(node, ast.ClassDef):
                        ...
                        if is_model:
                            ...
                            if "created_at" not in class_str:
                                violations.append(
                                    f"{py_file}:{node.lineno}:{node.name} sem created_at"
                                )
...
class TestAPIStandards:
    ...
    def test_routes_use_login_required(self):
        ...
        for py_file in get_python_files():
            ...
            if (
                "module" not in str(py_file).lower()
                and "api" not in str(py_file).lower()
            ):
                continue
            ...
            for i in range(len(lines) - 1):
                ...
                if (
                    "@" in current_line
                    and ".route" in current_line
                    and "methods=" in current_line
                ):
                    if any(
                        method in current_line
                        for method in [
                            "'POST'",
                            "'PUT'",
                            "'DELETE'",
        ...
        if violations:
            pytest.skip(
                f"Rotas sem @login_required (verificar se é intencional):\n"
                + "\n".join(violations[:10])
            )
...
class TestSecurityStandards:
    ...
    def test_no_eval_or_exec(self):
        ...
        assert (
            not violations
        ), f"eval() ou exec() encontrados (risco de segurança):\n" + "\n".join(
            violations
        )
