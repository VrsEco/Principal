...
from models import db, Company, Plan, Project, Indicator, Process, ProcessInstance
...
def data_health():
    ...
    if company_id:
        try:
            payload["counts"]["plans_by_company"] = Plan.query.filter(Plan.company_id == company_id).count()
        except Exception as exc:
            payload["warnings"].append(f"Falha plans_by_company: {exc}")

        # projects (tabela projects via ORM, filtrada via join de Plan)
        try:
            payload["counts"]["projects_by_company_via_plans"] = (
                Project.query.join(Plan).filter(Plan.company_id == company_id).count()
            )
        except Exception as exc:
            payload["warnings"].append(f"Falha projects_by_company_via_plans: {exc}")
