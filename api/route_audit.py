...
def get_all_routes():
    ...
    try:
        ...
        filter_type = request.args.get('filter', 'all')  # all, with_logging, without_logging, crud
        ...
        if filter_type == 'with_logging':
            ...
            routes = [r for r in routes if r['needs_logging'] and not r['has_auto_log'] and not r['has_manual_log']]
        ...
        return jsonify({
            'success': False,
            'message': f'Erro ao obter rotas: {str(e)}'
        }), 500
...
def enable_entity_logging(entity_type):
    ...
    try:
        ...
        return jsonify({
            'success': False,
            'message': f'Erro ao habilitar logging: {str(e)}'
        }), 500
...
def disable_entity_logging(entity_type):
    ...
    try:
        ...
        return jsonify({
            'success': False,
            'message': f'Erro ao desabilitar logging: {str(e)}'
        }), 500
