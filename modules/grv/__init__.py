...
def _build_display_name(code: Optional[str], name: Any, fallback: str) -> str:
    safe_code = (code or '').strip()
    safe_name = name.strip() if isinstance(name, str) else ''
    if safe_code:
        formatted_name = safe_name.upper() if safe_name else fallback.upper()
        return f"{safe_code} - {formatted_name}"
...
def grv_process_map_print(company_id: int):
    ...
    for area in raw_areas:
        ...
        area_entry = {
            'display_name': _build_display_name(area.get('code'), area.get('name'), 'Área'),
            'color': area_color,
        ...
        for macro in macros:
            ...
            macro_entry = {
                'display_name': _build_display_name(macro.get('code'), macro.get('name'), 'Macroprocesso'),
            ...
            for proc in processes:
                ...
                macro_entry['processes'].append({
                    'display_name': _build_display_name(proc.get('code'), proc.get('name'), 'Processo'),
