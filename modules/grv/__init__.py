...
def ensure_indicator_goals_schema(conn):
    ...
    if "goal_type" not in columns:
        cursor.execute(
            "ALTER TABLE indicator_goals ADD COLUMN goal_type TEXT DEFAULT 'single'"
        )
    ...
    if "evaluation_basis" not in columns:
        cursor.execute(
            "ALTER TABLE indicator_goals ADD COLUMN evaluation_basis TEXT DEFAULT 'value'"
        )
    ...
    if "okr_reference_label" not in columns:
        cursor.execute(
            "ALTER TABLE indicator_goals ADD COLUMN okr_reference_label TEXT"
        )
...
def normalize_goal_row(goal: dict) -> dict:
    ...
    evaluation = goal.get("evaluation_basis") or (
        "value" if goal_type == "single" else "sum"
    )
    ...
    goal["okr_reference_label"] = (
        okr_label.strip() if isinstance(okr_label, str) and okr_label.strip() else None
    )
...
def grv_dashboard():
    ...
    for company in companies:
        ...
        try:
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM processes WHERE company_id = %s", (company["id"],)
            )
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM process_areas WHERE company_id = %s",
                (company["id"],),
            )
            result = cursor.fetchone()
            if result:
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM company_projects WHERE company_id = %s",
                (company["id"],),
            )
            result = cursor.fetchone()
            if result:
        ...
        companies_context.append(
            {
                ...
                "client_code": (company.get("client_code") or "").strip()
                if isinstance(company.get("client_code"), str)
                else company.get("client_code"),
...
def grv_company_dashboard(company_id: int):
    ...
    quick_links = [
        ...
        {
            "label": "Estruturar processos",
            "href": url_for("grv.grv_process_map", company_id=company_id),
        },
    ]
...
def grv_process_map_print(company_id: int):
    ...
    def _format_datetime(
        value: Optional[datetime], with_time: bool = True
    ) -> Optional[str]:
        if not value:
            return None
...
def grv_process_map_pdf2_test(company_id: int):
    ...
    return jsonify(
        {"status": "OK", "message": "Test endpoint working", "company_id": company_id}
    )
...
def grv_process_modeling(company_id: int):
    ...
    if hasattr(db, "get_process_artifact_presence"):
        try:
            ...
            logger.info(
                f"[grv_process_modeling] Warning: failed to load artifact presence -> {exc}"
            )
            artifact_presence = {}
        ...
        process_groups[stage_slug].append(
            {
                ...
                "structuring_label": structuring_labels.get(
                    process.get("structuring_level") or "", "Sem avalia+â-º+â-úo"
                ),
...
def grv_process_detail(company_id: int, process_id: int):
    ...
    macro = next(
        (item for item in macros if item.get("id") == process.get("macro_id")), None
    )
    ...
    process_detail["structuring_label"] = structuring_labels.get(
        process.get("structuring_level") or "", "Sem avalia+â-º+â-úo"
    )
    ...
    stage_info = stage_lookup.get(
        process_detail.get("kanban_stage"), stage_lookup["inbox"]
    )
    ...
    stage_badge = stage_palette.get(
        process_detail.get("kanban_stage"), stage_palette["inbox"]
    )
...
def api_process_activities(company_id: int, process_id: int):
    ...
    if request.method == "GET":
        ...
        if not name:
            return (
                jsonify({"success": False, "error": "Activity name is required"}),
                400,
            )

        if not suffix:
            return (
                jsonify({"success": False, "error": "Activity suffix is required"}),
                400,
            )
...
def api_create_process_activity_entry(
    ...
    logger.info(
        f"=== FUN+ç+âO CHAMADA! company_id={company_id}, process_id={process_id}, activity_id={activity_id} ==="
    )

    db = get_db()

    logger.info(
        f"DEBUG: Recebendo requisi+º+úo para criar etapa - company_id={company_id}, process_id={process_id}, activity_id={activity_id}"
    )
    ...
    logger.info(
        f"DEBUG: Dados recebidos - layout={layout}, text_content={text_content[:50] if text_content else 'VAZIO'}, image_width={image_width}"
    )
            ...
            if new_entry:
                ...
                return (
                    jsonify(
                        {
                            "success": False,
                            "error": "Entry created but not found",
                            "message": "Falha ao buscar no banco",
                        }
                    ),
                    500,
                )
...
def grv_projects_portfolios(company_id: int):
    ...
    company = get_db().get_company(company_id) or {}
    return render_template(
        "grv_projects_portfolios.html",
        company=company,
        navigation=grv_navigation(),
        active_id="project-portfolios",
    )
...
def grv_project_shortcut(company_id: int, project_id: int):
    ...
    return redirect(
        url_for("grv.grv_project_manage", company_id=company_id, project_id=project_id)
    )
...
def grv_projects_analysis(company_id: int):
    ...
    for i, project_row in enumerate(projects_rows):
        ...
        project["completed_activities"] = len(
            [a for a in activities if a.get("status") == "completed"]
        )
        project["in_progress_activities"] = len(
            [a for a in activities if a.get("status") in ["executing", "waiting"]]
        )
        project["pending_activities"] = len(
            [a for a in activities if a.get("status") in ["inbox", "pending"]]
        )
...
@grv_bp.route(
    "/company/<int:company_id>/indicator-groups/form", defaults={"group_id": None}
)
...
def grv_indicators_list(company_id: int):
    ...
    # Get processes
    cursor.execute(
        "SELECT id, code, name FROM processes WHERE company_id = %s ORDER BY name",
        (company_id,),
    )
    processes = [dict(row) for row in cursor.fetchall()]
...
@grv_bp.route(
    "/company/<int:company_id>/indicators/form", defaults={"indicator_id": None}
)
...
def grv_indicator_form(company_id: int, indicator_id: Optional[int] = None):
    ...
    cursor.execute(
        "SELECT id, code, name FROM indicator_groups WHERE company_id = %s ORDER BY code",
        (company_id,),
    )
...
@grv_bp.route(
    "/company/<int:company_id>/indicator-goals/form", defaults={"goal_id": None}
)
...
@grv_bp.route(
    "/company/<int:company_id>/indicator-data/form", defaults={"record_id": None}
)
...
def api_update_indicator_group(company_id: int, group_id: int):
    ...
    try:
        cursor.execute(
            ...
            WHERE id = %s AND company_id = %s
        """,
            (data.get("name"), data.get("description"), group_id, company_id),
        )
...
def api_get_indicators(company_id: int):
    ...
    for indicator in indicators:
        ...
        if "group_code" in indicator:
            indicator["group_code"] = normalize_indicator_code(
                indicator.get("group_code")
            )
...
def api_get_indicator(company_id: int, indicator_id: int):
    ...
    if not row:
        return (
            jsonify({"success": False, "message": "Indicador n+â-úo encontrado"}),
            404,
        )
        ...
        if collaborators_value:
            ...
            try:
                ...
                if isinstance(collaborators_value, str):
                    collaborator_ids = [
                        int(item)
                        for item in collaborators_value.split(",")
...
def api_delete_indicator(company_id: int, indicator_id: int):
    ...
    try:
        ...
        conn.close()
        return jsonify(
            {"success": True, "message": "Indicador exclu+â-¡do com sucesso"}
        )
    ...
    try:
        ...
        for goal in goals:
            if "indicator_code" in goal:
                goal["indicator_code"] = normalize_indicator_code(
                    goal.get("indicator_code")
                )
    ...
    try:
        ...
        if period_start and period_end:
            try:
                ...
                if end_dt < start_dt:
                    raise ValueError(
                        "Periodo da meta invalido (data final anterior a inicial)."
                    )
        ...
        if row:
            return (
                jsonify({"success": True, "data": normalize_goal_row(dict(row))}),
                201,
            )
        return jsonify({"success": True, "data": {}}), 201
...
def api_update_indicator_goal(company_id: int, goal_id: int):
    ...
    try:
        cursor.execute(
            "SELECT id FROM indicator_goals WHERE id = %s AND company_id = %s",
            (goal_id, company_id),
        )
...
def api_get_indicator_data(company_id: int):
    ...
    for record in data_records:
        if "indicator_code" in record:
            record["indicator_code"] = normalize_indicator_code(
