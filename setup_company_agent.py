...
def create_sample_company_data():
    ...
    try:
        ...
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='company_data'")
        if not cursor.fetchone():
        ...
        cursor.execute("SELECT id FROM company_data WHERE plan_id = ?", (company_data['plan_id'],))
