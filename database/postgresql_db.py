...
class PostgreSQLDatabase(DatabaseInterface):
    ...
    def list_process_areas(self, company_id: int) -> List[Dict[str, Any]]:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT * FROM process_areas WHERE company_id = %s ORDER BY code, order_index, name",
            (company_id,),
        )
    ...
    def list_macro_processes(self, company_id: int) -> List[Dict[str, Any]]:
        ...
        cursor.execute(
            "SELECT * FROM macro_processes WHERE company_id = %s ORDER BY area_id, order_index, name",
            (company_id,),
        )
        rows = [dict(r) for r in cursor.fetchall()]
        conn.close()
    ...
    def list_processes(self, company_id: int) -> List[Dict[str, Any]]:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT * FROM processes WHERE company_id = %s ORDER BY code, order_index, name",
            (company_id,),
        )
        rows = []
        for row in cursor.fetchall():
    ...
    def delete_routine(self, routine_id: int) -> bool:
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE routines SET is_active = 0 WHERE id = %s", (routine_id,)
            )
            conn.commit()
            conn.close()
            return cursor.rowcount > 0
    ...
    def delete_routine_trigger(self, trigger_id: int) -> bool:
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE routine_triggers SET is_active = 0 WHERE id = %s", (trigger_id,)
            )
            conn.commit()
            conn.close()
            return cursor.rowcount > 0
